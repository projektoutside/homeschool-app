...
Task 7: fix round 3 complete (removed only the out-of-scope late balance test surface, preserved the pre-existing Task 8 mono-roster loss contract, and revalidated the early exact-metric campaign plus config/simulation/combat suites without introducing new late config/strategy drift).

Task 8 Ruling (2026-08-20): extended ownership narrowly to `public/Games/DefenderChampion/src/config/defenders.js` plus exact config/balance coverage because the mono Rune reinvestment contract was impossible under centerline combat authority. Measured root cause: on Levels 7 and 10 every grass-center to road-center tangent was 80px while Rune tier-0 range was 72px, so the tower recorded zero legal shots, zero damage, and zero bounty despite legal placement. Applied the smallest monotonic range increase to `[81, 90, 100]`, kept damage/costs/roles unchanged, and rewrote the mono scorer to use the authoritative centerline path only. Cost/risk: modest global Rune coverage buff; late-only recheck required because early exact metrics must remain unchanged and mono/mixed late balance can shift materially.

Task 8 Ruling (2026-08-20): use authored late composition and cadence pressure before further health scaling. Level 7 replaces the ten-enemy wave-5 Shellguard group with a zero-interval Skitter burst and moves the finale's 32-Skitter group to zero delay/interval; Level 10 replaces the authored three-enemy wave-3 Crusher group with three Ironhide Warlords at a 114-tick interval. Every group count remains unchanged, the 18-living/three-attacker authorities remain exact, and Levels 1–6 retain literal metrics. Cost/risk: the late campaign becomes more burst-oriented, so exact runtime cap sampling and mixed-strategy footprints must remain locked by tests.

Task 8 Ruling (2026-08-20): set Ironhide Warlord movement to 42 and castle damage to three as the smallest boss-identity pressure needed for a terminal three-heart breach after the Level 7 composition pass. Health, armor, bounty, attack damage/cadence, rally thresholds, plate thresholds, and telegraphs remain unchanged. The enemy occurs only in the late authored campaign after the Level 10 composition substitution. Cost/risk: any future mixed strategy must kill every unblocked Warlord; exact Level 7/10 victories and all eight mono defeats therefore remain literal deterministic evidence.

Task 8 Ruling (2026-08-20): retain the config contract that late strategies upgrade before a speculative third build. The final Level 7 artillery and both Level 10 strategies add an accepted early upgrade and redistribute later cells/ticks rather than weakening chronology or due-command assertions. Level 10 balanced permanently loses `r1c2` at tick 3444 and pays for its same-cell rebuild at tick 35000; Level 7 balanced permanently loses `r2c2` at tick 11647 and pays for its rebuild at tick 18000. Cost/risk: terminal ticks and spend leaders changed, so all eight late fixture metrics are asserted as literals and verified in two independent full passes.

Task 8: complete (commits `ffe32b5`..`4dbce08`, review clean; eight literal Levels 7–10 mixed victories with zero due-command rejections; eight terminal Level 7/10 mono defeats with at least three post-tick-zero purchases; 10/10 no-build defeats; exact Level 4/7/10 rebuild evidence; caps 18/3; final legacy strategy adapter and deprecated placement references removed; two full deterministic balance passes plus core/runtime/build/search gates green).

Base before Task 9: `4dbce08f057295b60a26df1fc120e928cc5ef424`.

Task 9 Ruling: the extracted brief's report path omits the plan's `-portrait` suffix and therefore points at a sibling/nonexistent workspace. Write the Task 9 report to this plan's authoritative workspace, `.superpowers/sdd/2026-08-19-defender-champion-square-grid-portrait/task-9-report.md`, and do not create or modify the sibling path. — The subagent-driven-development workspace identity is authoritative and the report must survive compaction beside this plan's ledger/briefs. — Cost if wrong: a reviewer expecting the literal typo path would need to be redirected to the correct plan-owned report.

Task 9: complete (four rendered defects closed TDD-first: rejected pointer focus mutation, ordinary pause loop freeze/queued duplicate pause calls, 40px Start hit target, and clipped 360x640 contextual actions; a clean-Windows-checkout catalog CRLF/LF parity failure exposed by the first exact-candidate gate was fixed with a single generated-file `eol=lf` attribute; final bundle regenerated; 217/217 Defender tests, typecheck, lint, 82-entry game audit, asset audit, and diff gate green; separate ordinary Level 1 real-combat victory plus all-ten QA-accelerated deterministic campaign outcomes, one rendered no-build defeat, all six portrait viewports, representative rotations, Level 7/10 hard waves/death/rebuild/thresholds, reduced motion, resilience, host/embedded/standalone paths, 18-of-18 projection/pool reset, and 41 screenshots green; desktop/mobile-emulated p95 13.36/13.37ms; payload 14,801,907 bytes; fresh exact-candidate isolated `npm run check` green; owned browser/server/verifier cleaned; protected hashes unchanged; physical/device truth recorded in `task-9-report.md`).

Task 9: independent review fix round 1 complete (dense Level 7/10 unit stacking/clipping reproduced in existing evidence; the real Level 10 cap regression now measures all 18 individual body rectangles, full board containment, one-third maximum rendered occlusion, and fixed 38/52 CSS-pixel unit/boss floors; normalization-metadata body projection plus deterministic bidirectional path-slot presentation keeps all units individually readable without changing FIFO/cap-18/attacker-3/square-grid/balance authority; browser REDs at 0.42257/0.92625 overlap and 22.68ms p95 were fixed without weakening animation or thresholds; one-session replay green for separate ordinary Level 1 real combat, all-ten QA-accelerated campaign outcomes, six portrait sizes, Level 7 zero-interval delivery, Level 10 18-living desktop/390x844 and Dread thresholds, 41 fresh screenshots, zero unexpected diagnostics, 13.35/18.67ms desktop/mobile p95, and 14,806,597-byte payload; exact-commit result recorded in `task-9-report.md`; physical/emulated/code-reviewed truth unchanged).

Task 9: fix round 1/5 (3 addressed, 0 open — dense unit containment/readability, exact repository count/duration, and Level 1 evidence wording; commits `1fbfe35`..`11eaa23`).

Task 9: complete (commits `4dbce08`..`11eaa23`, review clean).

Final Fix Ruling (2026-08-20): visual/spatial truth and authoritative FIFO admission/backpressure are binding; the prior exact terminal ticks were regression sentinels rather than player-facing authority. Retain the deterministic per-lane early-gate admission guard that keeps 390px bodies contained, at or below one-third overlap, at truthful gate/queue/movement progress, and leaves excess requests pending instead of visually teleporting live enemies. Do not retune combat merely to recover the former ticks. Cost if wrong: evidence-backed campaign completion ticks and the Level 8 balanced score shift and downstream literal expectations must be refreshed; all outcomes, hearts, economy, footprints, paid rebuilds, 20 mixed wins, 10 no-build losses, 8 mono losses, cap 18/max three, boss mechanics, and difficulty semantics remain exact. Final stable rebaseline and two-pass proof are recorded in `final-fix-report.md`.

Final Fix Ruling (2026-08-20): no native ScreenOrientation plugin is installed, while adding a runtime dependency or whole-app AndroidManifest/Info.plist lock would violate the protected/no-dependency constraints. Use the Defender-only same-origin request/release seam and the host adapter's injected-native, already-registered Capacitor, and browser capabilities; return unsupported when none exists and keep the landscape overlay authoritative. Cost if wrong: a shell without one of those capabilities continues safely through the overlay but cannot physically lock orientation until it supplies the tested game-scoped bridge.

Final fix pre-commit verification complete: all nine review findings closed TDD-first; final 40/80 per-lane admission metrics passed two independent balance runs (`160904.233 ms` and `160402.4244 ms`); final Defender suite passed 228/228 in `163403.6474 ms`; exact bundle parity, typecheck, lint, 82-entry game audit, 763-file asset audit, and whitespace gate passed; final headed Chromium dense/landscape/input/accessibility/endpoint/performance replay passed with `13.38/13.38 ms` p95; six-viewport motion/resilience replay passed; payload was 51 files / `14,816,232` bytes; protected hashes remained exact. Final report: `final-fix-report.md`.

Final fix exact-candidate round 1: detached candidate `2ddb5208d4ade0c6252c4256e4a7deb62d301332` passed 264/265 repository tests but correctly failed the new exact bundle-byte gate because Windows clean checkout converted the generated Defender bundle to CRLF (`5B7E...B191` versus in-memory LF `6D5F...F1DA`). Added the narrow `.gitattributes` LF rule for this generated artifact; the failed verifier junction/worktree was removed before creating the amended candidate.

Final fix exact-candidate round 2: LF-normalized candidate `c7d984f823719f2125e16232edd0401c66ebee6f` passed clean-checkout bundle parity 2/2 and full detached `npm run check`: 265/265 repository tests in `163510.3774 ms`, typecheck, lint, 82-entry game audit, 763-file asset audit, deterministic generators, Quiz/Defender bundle builds, and Vite build; total `204213.8 ms`. Its junction/worktree was cleaned. The final report/ledger-only amend is rerun through the same detached exact gate before handoff.
