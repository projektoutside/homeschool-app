...
All ten QA-accelerated deterministic victory replays completed. These are rendered campaign/checkpoint harness outcomes, not ten ordinary end-to-end combat runs:
...
Level 1's `maxLiving: 0` is QA-acceleration bookkeeping: its accepted command was accelerated before a living-enemy checkpoint was sampled. It is not described as real rendered combat. The separate ordinary-mode Level 1 replay used real pointer/touch-equivalent product input, visibly rendered live attacks, and reached victory without `window.__defenderChampion`.
...
## Independent review fix round 1 — individual unit readability

The independent review correctly found that the prior `level-10-18-living-desktop.png`, its mobile counterpart, and the Level 7 queued-delivery evidence allowed bodies to stack and clip even though the old browser gate checked only 18 views and fixed scale. The fix is presentation-only: simulation position, FIFO/cap authority, attacker selection, wave timing, square geometry, and balance were not changed.

### Executable RED/GREEN record

- RED 1: the strengthened real Level 10 cap test required 18 measured body rectangles, full 720x960 board containment, and pair overlap at or below one third. Before production support it failed with `bodyRectangles: 0`, `contained: false`, and `maximumOverlapWithinLimit: false` (`1 failed`, `161.9278 ms`).
- GREEN 1: each atlas character now uses its normalization metadata to project a measured body rectangle. A deterministic path/lane layout keeps bodies within the visible world and reserves readable individual slots without changing any simulation entity. The focused readable-art/projection/cap/shared-transform set passed `4/4` in `191.3857 ms`.
- Browser RED 2: the first consolidated campaign replay caught authored lunge motion raising a Level 10 Dread 50% threshold pair to `0.4225694444444438` overlap. Resting slots were tightened to zero overlap so the motion budget still remains below the one-third rendered ceiling.
- Browser RED 3: a packed near-castle threshold then showed `0.9262493799603164` overlap because the forward-only slot search exhausted the remaining path. The existing Level 10 cap regression was extended with an 18-body castle-end fixture; it witnessed `1 !== 0` maximum overlap. The layout now searches the nearest free path progress in both directions while preserving stable entity identity.
- Performance RED/GREEN: the first bidirectional version performed redundant clamped endpoint checks and failed the desktop replay at `22.68 ms` p95. Eliminating repeated candidates restored the exact workload to `13.35 ms` p95 while keeping 18/18 views and zero measured overlap.
- Final focused Level 10 cap test: PASS (`1/1`, `224.2646 ms`). No geometry, balance, cap, scale, orientation, accessibility, or minimum-art threshold was weakened.

### Final headed-browser replay

The complete one-session headed Windows Chromium replay passed from `2026-08-20T11:05:02` through `2026-08-20T11:08:57`: ordinary Level 1 real-combat victory; six portrait viewports; ten QA-accelerated deterministic victories; rendered no-build defeat; Level 7 zero-interval groups; Level 10 cap/Warlords/Dread thresholds; reduced motion; lifecycle, embedded, standalone, denial, corrupt-save, and injected-failure resilience. It produced 41 fresh screenshots. Ordinary, matrix, campaign, performance, lifecycle, embedded, standalone, storage/audio denial, corrupt-save, and reduced-motion diagnostics had zero unexpected console warnings/errors, page errors, failed requests, or bad responses; only the deliberately injected missing-asset contexts emitted their expected failures.

Fresh evidence inspected visually:

- `level-7-finale-queued-skitter-delivery.png`
- `level-10-18-living-desktop.png`
- `level-10-18-living-mobile-390x844.png`
- `level-10-dread-threshold-75.png`, `-50.png`, and `-25.png`

At the exact cap checkpoint, desktop and emulated 390x844 Chromium each exposed 18 living snapshots, 18 projected body rectangles, zero missing views, zero measured overlap, and fixed density scale. Minimum standard body height was `78.370 CSS px` desktop and `41.932 CSS px` mobile, above the fixed 38-pixel floor; Dread threshold captures had minimum standard height `89.520–95.398 CSS px` and minimum boss height `121.013 CSS px`, above the fixed 52-pixel boss floor. Desktop/mobile p95 was `13.35/18.67 ms`, within the 60/30 FPS gates. The final payload was 51 files / `14,806,597` raw bytes, leaving `193,403` bytes below the strict cap. Final generated Defender bundle SHA-256: `172917A7E36BB5026ADA2345A27AF8577342AB14AA182EDAF22EFEAEC76DBFD1`.

Primary-worktree verification after the fix: Defender tests PASS (the complete run plus the subsequently folded castle-end assertion retained the final suite count), `audit:games` PASS at 82 entries, `audit:assets` PASS at 763 files with report-only pre-existing notices, typecheck PASS, lint PASS, bundle build PASS, and `git diff --check` PASS. The first code-complete isolated candidate `0b1fb074c7ae71505cad6c9ddf9062ad4a55b718` passed the full `npm run check`: exact repository test phase `254/254` passed, `0` failed, `0` skipped, duration `149245.5044 ms`; typecheck, lint, 82-entry game audit, 763-file asset audit, deterministic catalog/build generators, Quiz bundle, Defender bundle, and Vite production build all passed. After recording that evidence in this report, the report-only amend was rerun through the same exact detached gate before final cleanup.

Physical truth is unchanged: headed Windows desktop Chromium was physical; phone/tablet sizes, touch, rotations, reduced motion, and mobile performance were emulated. Safari, Android-browser-specific behavior, and Capacitor/native-shell behavior remain automated/code-reviewed only.
