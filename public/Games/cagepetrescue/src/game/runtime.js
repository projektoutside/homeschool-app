...
import { bindUpgradesPanelButton } from '../ui/upgrades-panel-button.js';
        ...
        let goblinSpriteBundle = null;
        let goblinSpriteLoadPromise = null;
        let goblinSpriteLoadFailed = false;
        let activeGoblinIntro = null;
        let roundIntroSequence = 0;
        ...
        const goblinLaunchBadge = document.getElementById('goblin-launch-badge');
        ...
        const mathWizardActionButton = document.getElementById('math-wizard-button');
        ...
        const elementWorldPointScratch = new THREE.Vector3();
        const elementWorldDirectionScratch = new THREE.Vector3();
        const goblinBoundsScratch = new THREE.Box3();
        const goblinBoundsSizeScratch = new THREE.Vector3();
        const goblinBoundsCenterScratch = new THREE.Vector3();
        const goblinTargetScratch = new THREE.Vector3();
        const goblinPerchScratch = new THREE.Vector3();
        const goblinHoverScratch = new THREE.Vector3();
        const goblinMidScratch = new THREE.Vector3();
        const goblinExitScratch = new THREE.Vector3();
        const goblinBezierScratchA = new THREE.Vector3();
        const goblinBezierScratchB = new THREE.Vector3();
        const goblinBezierScratchC = new THREE.Vector3();
        const goblinDirectionScratch = new THREE.Vector3();
        const goblinOffsetScratch = new THREE.Vector3();
        const goblinLookTargetScratch = new THREE.Vector3();
        const goblinGripTargetScratch = new THREE.Vector3();
        const goblinGripLeftScratch = new THREE.Vector3();
        const goblinGripRightScratch = new THREE.Vector3();
        const goblinGripMidScratch = new THREE.Vector3();
        const goblinGripCorrectionScratch = new THREE.Vector3();
        const goblinBoneEulerScratch = new THREE.Euler(0, 0, 0, 'XYZ');
        const goblinBoneQuaternionScratch = new THREE.Quaternion();
        const GOBLIN_SCENE_ROOT_NAME = 'cage-pet-rescue-goblin';
        const GOBLIN_LEGACY_SCENE_ROOT_NAME = 'cage-pet-rescue-goblin-intro';
        ...
        function getElementWorldTarget(element, targetZ = 0, sourceContainer = gameContainer) {
            if (!camera || !sourceContainer || !element) {
                return null;
            }

            const gameRect = sourceContainer.getBoundingClientRect();
            ...
            const worldPoint = elementWorldPointScratch.set(normalizedX, normalizedY, 0.5).unproject(camera);
            const direction = elementWorldDirectionScratch.copy(worldPoint).sub(camera.position).normalize();
        ...
        function getMathDeckWorldTarget(mathBox, targetZ = 0) {
            return getElementWorldTarget(mathBox, targetZ, gameContainer);
        }
        ...
        function updateTimerDisplay() {
            ...
            timerDisplay.classList.remove('round-intro', 'round-intro-final');
        ...
        function updateHUD() {
            ...
            const now = Date.now();
            const gameplayControlsLocked = gameState !== 'PLAYING' || isGamePaused;
            for (const action in actionButtons) {
                ...
                if (gameplayControlsLocked || remainingCooldown > 0 || !hasEnoughEnergy) {
                    ...
                    if (!gameplayControlsLocked && !hasEnoughEnergy && remainingCooldown <= 0) { button.style.outline = '2px solid orange'; }
  ...
  function ensureButtonClickability() {
    ...
    document.querySelectorAll('.mc-choice-btn').forEach(btn => {
      ...
      btn.style.zIndex = '20';
        ...
        function scheduleNextLightningStrike({ initial = false } = {}) {
            ...
            const minimumDelay = initial
                ? CONFIG.LIGHTNING_MIN_INTERVAL / 2
                : CONFIG.LIGHTNING_MIN_INTERVAL;
            const nextStrikeDelay = minimumDelay + Math.random() * (CONFIG.LIGHTNING_MAX_INTERVAL - CONFIG.LIGHTNING_MIN_INTERVAL);
            lightningTimeoutId = setTimeout(triggerLightning, nextStrikeDelay);
        }

        function clearCloudSpawnTimer() {
            if (cloudSpawnTimeoutId) {
                clearTimeout(cloudSpawnTimeoutId);
                cloudSpawnTimeoutId = null;
            }
        ...
        function clearLegendaryJoltTimer() {
        ...
        function beginGameplayTimerHold() {
            if (realTimeTimerStart > 0 && gameplayTimerHoldStart === 0) {
                gameplayTimerHoldStart = performance.now();
            }
        }

        function endGameplayTimerHold() {
            if (realTimeTimerStart > 0 && gameplayTimerHoldStart > 0) {
                realTimeTimerStart += performance.now() - gameplayTimerHoldStart;
            }
            gameplayTimerHoldStart = 0;
        }

        function resetGameplayTimerBaseline() {
            const now = performance.now();
            realTimeTimerStart = now;
            realTimeElapsed = gameTime;
            initialGameTime = gameTime;
        ...
        function setTimerDisplayFromGameTime() {
            const minutes = Math.floor(gameTime / 60);
            const seconds = Math.floor(gameTime % 60);
            timerDisplay.textContent = `${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`;
            timerDisplay.classList.remove('round-intro', 'round-intro-final');
        ...
        function startRoundIntroTimer() {}

        function stopRoundIntroTimer() {
            timerDisplay.classList.remove('round-intro', 'round-intro-final');
        }
        ...
        function setRoundControlsEnabled(enabled) {
            ...
            if (mathWizardActionButton) {
                const cooldownDisplay = mathWizardActionButton.querySelector('.cooldown-timer');
                if (!enabled) {
                    mathWizardActionButton.disabled = true;
                } else {
                    mathWizardActionButton.disabled = mathWizardCooldown > 0 || gameState !== 'PLAYING';
                    if (cooldownDisplay && mathWizardCooldown <= 0) {
            ...
            if (typeof updateLockPickButtonStates === 'function') {
                updateLockPickButtonStates();
            }
        }

        function primeGoblinAsset() {
            void ensureGoblinSpriteAssetsLoaded();
        }

        function loadGoblinSpriteTexture(textureUrl) {
            return new Promise((resolve, reject) => {
                ...
                loader.load(
                    ...
                    (texture) => {
                        ...
                        texture.anisotropy = renderer?.capabilities ? Math.min(4, renderer.capabilities.getMaxAnisotropy()) : 1;
        ...
        function normalizeGoblinAnimationConfig(animationConfig, key) {
            if (!animationConfig || typeof animationConfig !== 'object') {
                throw new Error(`Missing goblin animation config for "${key}".`);
            }

            const frameCount = Number(animationConfig.frameCount);
            const columns = Number(animationConfig.columns);
            const rows = Number(animationConfig.rows);
            const fps = Number(animationConfig.fps);
            if (!animationConfig.atlasPath || !frameCount || !columns || !rows || !fps) {
                throw new Error(`Goblin animation "${key}" is missing atlasPath/frameCount/columns/rows/fps.`);
            }

            return {
                atlasPath: animationConfig.atlasPath,
                reviewGifPath: animationConfig.reviewGifPath || null,
                frameCount,
                columns,
                rows,
                fps,
                loop: Boolean(animationConfig.loop),
                frameWidth: Number(animationConfig.frameWidth) || 0,
                frameHeight: Number(animationConfig.frameHeight) || 0,
            };
        }

        async function ensureGoblinSpriteAssetsLoaded() {
            if (goblinSpriteBundle) {
                return goblinSpriteBundle;
            }

            if (goblinSpriteLoadFailed) {
                return null;
            }

            if (!goblinSpriteLoadPromise) {
                goblinSpriteLoadPromise = (async () => {
                    try {
                        const manifestResponse = await fetch(CONFIG.GOBLIN_INTRO.MANIFEST_URL);
                        if (!manifestResponse.ok) {
                            throw new Error(`HTTP ${manifestResponse.status}`);
                        }

                        const manifest = await manifestResponse.json();
                        const frameWidth = Number(manifest?.frameSize?.width);
                        const frameHeight = Number(manifest?.frameSize?.height);
                        const anchorX = Number(manifest?.anchorPx?.x);
                        const anchorY = Number(manifest?.anchorPx?.y);
                        if (!frameWidth || !frameHeight || !Number.isFinite(anchorX) || !Number.isFinite(anchorY)) {
                            throw new Error('Goblin sprite manifest is missing frameSize/anchorPx.');
                        }

                        const manifestUrl = new URL(CONFIG.GOBLIN_INTRO.MANIFEST_URL, window.location.href);
                        const hoverConfig = normalizeGoblinAnimationConfig(manifest.animations?.hover, 'hover');
                        const attackConfig = normalizeGoblinAnimationConfig(manifest.animations?.attack, 'attack');

                        const [hoverTexture, attackTexture] = await Promise.all([
                            loadGoblinSpriteTexture(new URL(hoverConfig.atlasPath, manifestUrl).href),
                            loadGoblinSpriteTexture(new URL(attackConfig.atlasPath, manifestUrl).href),
                        ]);

                        goblinSpriteBundle = {
                            manifest: {
                                characterId: manifest.characterId || 'cage-pet-rescue-goblin-fairy',
                                frameSize: { width: frameWidth, height: frameHeight },
                                anchorPx: { x: anchorX, y: anchorY },
                                collisionBoxPx: manifest.collisionBoxPx || null,
                            },
                            animations: {
                                hover: { ...hoverConfig, texture: hoverTexture },
                                attack: { ...attackConfig, texture: attackTexture },
                            },
                        };
                        return goblinSpriteBundle;
                    } catch (error) {
                        goblinSpriteLoadFailed = true;
                        console.error('Failed to load Cage Pet Rescue goblin sprite assets.', error);
                        return null;
                    }
                })();
            }

            return goblinSpriteLoadPromise;
        }

        function blendAngle(current, target, amount = 1) {
            const delta = Math.atan2(Math.sin(target - current), Math.cos(target - current));
            return current + (delta * amount);
        }

        function createMagicActorAura(disposeCallbacks, options = {}) {
            ...
            const mainLight = new THREE.PointLight(
                ...
                options.lightDistance ?? 3,
                2
            );
            group.add(mainLight);

            const coreGeometry = new THREE.SphereGeometry(options.coreRadius ?? 0.05, 14, 14);
            const coreMaterial = new THREE.MeshBasicMaterial({
                color: options.coreColor ?? 0xffffff,
                transparent: true,
                opacity: options.coreOpacity ?? 0.18,
            ...
            const auraGeometry = new THREE.SphereGeometry(options.auraRadius ?? 0.14, 18, 18);
            const auraMaterial = new THREE.MeshBasicMaterial({
                color: options.auraColor ?? 0x66bbff,
                transparent: true,
                opacity: options.auraOpacity ?? 0.1,
                blending: THREE.AdditiveBlending,
                depthWrite: false,
                side: THREE.BackSide,
            });
            const aura = new THREE.Mesh(auraGeometry, auraMaterial);
            const auraScale = options.auraScale || [1, 1, 1];
            aura.scale.set(auraScale[0], auraScale[1], auraScale[2]);
            group.add(aura);

            const shellGeometry = new THREE.SphereGeometry(options.shellRadius ?? 0.2, 20, 20);
            const shellMaterial = new THREE.MeshBasicMaterial({
                color: options.shellColor ?? 0x3388ff,
                transparent: true,
                opacity: options.shellOpacity ?? 0.08,
                blending: THREE.AdditiveBlending,
                depthWrite: false,
                side: THREE.BackSide,
            });
            const shell = new THREE.Mesh(shellGeometry, shellMaterial);
            const shellScale = options.shellScale || [1, 1, 1];
            shell.scale.set(shellScale[0], shellScale[1], shellScale[2]);
            group.add(shell);

            const haloGeometry = new THREE.RingGeometry(
                options.haloInnerRadius ?? 0.06,
                options.haloOuterRadius ?? 0.14,
                24
            );
            const haloMaterial = new THREE.MeshBasicMaterial({
                color: options.haloColor ?? 0xbfe4ff,
                transparent: true,
                opacity: options.haloOpacity ?? 0.08,
                blending: THREE.AdditiveBlending,
                depthWrite: false,
                side: THREE.DoubleSide,
            });
            const halo = new THREE.Mesh(haloGeometry, haloMaterial);
            const haloPosition = options.haloPosition || [0, 0.02, -0.08];
            halo.position.set(haloPosition[0], haloPosition[1], haloPosition[2]);
            halo.rotation.x = options.haloRotationX ?? 0;
            halo.rotation.y = options.haloRotationY ?? 0;
            halo.rotation.z = options.haloRotationZ ?? 0;
            group.add(halo);

            disposeCallbacks.push(() => coreGeometry.dispose());
            disposeCallbacks.push(() => auraGeometry.dispose());
            disposeCallbacks.push(() => shellGeometry.dispose());
            disposeCallbacks.push(() => haloGeometry.dispose());
            disposeCallbacks.push(() => coreMaterial.dispose());
            disposeCallbacks.push(() => auraMaterial.dispose());
            disposeCallbacks.push(() => shellMaterial.dispose());
            disposeCallbacks.push(() => haloMaterial.dispose());
            ...
            return {
                ...
                halo,
                visibilityAlpha: 1,
                baseLightIntensity: options.lightIntensity ?? 1,
                lightPulse: options.lightPulse ?? 0.2,
                coreOpacity: options.coreOpacity ?? 0.18,
                corePulse: options.corePulse ?? 0.06,
                auraOpacity: options.auraOpacity ?? 0.1,
                auraPulse: options.auraPulse ?? 0.05,
                shellOpacity: options.shellOpacity ?? 0.08,
                shellPulse: options.shellPulse ?? 0.05,
                haloOpacity: options.haloOpacity ?? 0.08,
                haloPulse: options.haloPulse ?? 0.04,
                pulseSpeed: options.pulseSpeed ?? 3.2,
                auraBaseScale: new THREE.Vector3(auraScale[0], auraScale[1], auraScale[2]),
                shellBaseScale: new THREE.Vector3(shellScale[0], shellScale[1], shellScale[2]),
                auraScalePulse: options.auraScalePulse ?? 0.08,
                shellScalePulse: options.shellScalePulse ?? 0.12,
            };
        }

        function updateMagicActorAura(nodes, phaseTime, {
            intensityBoost = 0,
            shellPulse = 0,
            haloBoost = 0,
            coreBoost = 0,
            ...
            const visibilityAlpha = nodes.visibilityAlpha ?? 1;
            const pulse = Math.sin(phaseTime * nodes.pulseSpeed) * 0.5 + 0.5;
            const shellBoost = shellPulse * 0.08;

            if (nodes.mainLight) {
                nodes.mainLight.intensity = (nodes.baseLightIntensity + (pulse * nodes.lightPulse) + intensityBoost) * visibilityAlpha;
            }

            if (nodes.core?.material) {
                nodes.core.material.opacity = (nodes.coreOpacity + (pulse * nodes.corePulse) + coreBoost) * visibilityAlpha;
            }

            if (nodes.aura?.material) {
                nodes.aura.material.opacity = (nodes.auraOpacity + (pulse * nodes.auraPulse) + (shellPulse * 0.04)) * visibilityAlpha;
                const auraScale = 1 + (pulse * nodes.auraScalePulse) + (shellPulse * 0.05);
                nodes.aura.scale.set(
                    nodes.auraBaseScale.x * auraScale,
                    nodes.auraBaseScale.y * (auraScale + (shellPulse * 0.05)),
                    nodes.auraBaseScale.z * (auraScale * 0.96)
                );
            }

            if (nodes.shell?.material) {
                nodes.shell.material.opacity = (nodes.shellOpacity + (pulse * nodes.shellPulse) + shellBoost) * visibilityAlpha;
                const shellScale = 1 + (pulse * nodes.shellScalePulse) + (shellPulse * 0.08);
                nodes.shell.scale.set(
                    nodes.shellBaseScale.x * shellScale,
                    nodes.shellBaseScale.y * (shellScale + (shellPulse * 0.06)),
                    nodes.shellBaseScale.z * (shellScale * 0.94)
                );
            }

            if (nodes.halo?.material) {
                nodes.halo.material.opacity = (nodes.haloOpacity + (pulse * nodes.haloPulse) + haloBoost) * visibilityAlpha;
            }
        }

        function createFairyWingGeometry() {
            const wingShape = new THREE.Shape();
            wingShape.moveTo(0, 0);
            wingShape.bezierCurveTo(0.18, 0.08, 0.3, 0.26, 0.26, 0.46);
            wingShape.bezierCurveTo(0.18, 0.74, -0.02, 0.98, -0.18, 1.14);
            wingShape.bezierCurveTo(-0.28, 0.94, -0.3, 0.64, -0.22, 0.34);
            wingShape.bezierCurveTo(-0.14, 0.14, -0.06, 0.03, 0, 0);
            return new THREE.ShapeGeometry(wingShape, 24);
        }

        function createProceduralFairyRig(variantKey = 'STRESS') {
            const baseConfig = CONFIG.FAIRY_VISUALS.BASE;
            const variantConfig = CONFIG.FAIRY_VISUALS[variantKey];
            if (!scene || !variantConfig) {
                return null;
            }

            const scale = baseConfig.BODY_SCALE;
            const disposeCallbacks = [];
            const fadeMaterials = [];
            const fadeLights = [];

            const registerMaterial = (material) => {
                material.userData.magicBaseOpacity = typeof material.opacity === 'number' ? material.opacity : 1;
                fadeMaterials.push(material);
                disposeCallbacks.push(() => material.dispose());
                return material;
            };
            const registerLight = (light) => {
                light.userData.magicBaseIntensity = light.intensity;
                fadeLights.push(light);
                return light;
            };

            const root = new THREE.Group();
            root.name = `cage-pet-rescue-${variantKey.toLowerCase()}-fairy`;
            const bodyPivot = new THREE.Group();
            root.add(bodyPivot);

            const bodyMaterial = registerMaterial(new THREE.MeshPhysicalMaterial({
                color: variantConfig.BODY_COLOR,
                emissive: variantConfig.BODY_EMISSIVE,
                emissiveIntensity: variantConfig.BODY_EMISSIVE_INTENSITY,
                roughness: 0.18,
                metalness: 0.08,
                clearcoat: 0.42,
            ...
            const lowerBodyMaterial = registerMaterial(new THREE.MeshPhysicalMaterial({
                color: variantConfig.BODY_COLOR,
                emissive: variantConfig.BODY_EMISSIVE,
                emissiveIntensity: variantConfig.BODY_EMISSIVE_INTENSITY * 0.88,
                roughness: 0.12,
                metalness: 0.04,
                clearcoat: 0.35,
            ...
            const limbMaterial = registerMaterial(new THREE.MeshPhysicalMaterial({
                color: variantConfig.BODY_COLOR,
                emissive: variantConfig.BODY_EMISSIVE,
                emissiveIntensity: variantConfig.BODY_EMISSIVE_INTENSITY * 0.72,
                roughness: 0.22,
                metalness: 0.06,
                clearcoat: 0.25,
                transparent: true,
            ...
            const wingMaterial = registerMaterial(new THREE.MeshPhysicalMaterial({
                color: variantConfig.WING_COLOR,
                emissive: variantConfig.WING_EMISSIVE,
                emissiveIntensity: 0.52,
                transparent: true,
                opacity: variantConfig.WING_OPACITY,
                side: THREE.DoubleSide,
                roughness: 0.08,
                metalness: 0.04,
                transmission: 0.28,
            ...
            const rearWingMaterial = registerMaterial(wingMaterial.clone());
            rearWingMaterial.opacity = Math.max(0.18, variantConfig.WING_OPACITY - 0.12);
            rearWingMaterial.emissiveIntensity = 0.36;
            rearWingMaterial.userData.magicBaseOpacity = rearWingMaterial.opacity;

            const haloMaterial = registerMaterial(new THREE.MeshBasicMaterial({
                color: variantConfig.HALO_COLOR,
                transparent: true,
                opacity: 0.55,
                blending: THREE.AdditiveBlending,
                depthWrite: false,
            }));
            const eyeMaterial = registerMaterial(new THREE.MeshBasicMaterial({
                color: variantConfig.AURA_CORE_COLOR,
                transparent: true,
                opacity: 0.86,
                blending: THREE.AdditiveBlending,
                depthWrite: false,
            }));
            const sparkleMaterial = registerMaterial(new THREE.MeshBasicMaterial({
                color: variantConfig.SPARKLE_COLOR,
                transparent: true,
                opacity: 0.75,
                blending: THREE.AdditiveBlending,
                depthWrite: false,
            ...
            const headGeometry = new THREE.SphereGeometry(baseConfig.HEAD_RADIUS * scale, 18, 18);
            const torsoGeometry = new THREE.SphereGeometry(baseConfig.TORSO_RADIUS * scale, 18, 18);
            const lowerBodyGeometry = new THREE.ConeGeometry(baseConfig.TORSO_RADIUS * scale * 1.35, baseConfig.LOWER_BODY_HEIGHT * scale, 18);
            const armGeometry = new THREE.CylinderGeometry(
                baseConfig.ARM_RADIUS * scale * 0.9,
                baseConfig.ARM_RADIUS * scale,
                baseConfig.ARM_LENGTH * scale,
                10
            );
            const handGeometry = new THREE.SphereGeometry(baseConfig.HAND_RADIUS * scale, 12, 12);
            const wingGeometry = createFairyWingGeometry();
            const haloGeometry = new THREE.TorusGeometry(
                baseConfig.HALO_RADIUS * scale,
                baseConfig.HALO_TUBE_RADIUS * scale,
                12,
                24
            );
            const eyeGeometry = new THREE.SphereGeometry(0.018 * scale, 10, 10);
            const sparkleGeometry = new THREE.OctahedronGeometry(baseConfig.SPARKLE_SIZE * scale, 0);

            disposeCallbacks.push(() => headGeometry.dispose());
            disposeCallbacks.push(() => torsoGeometry.dispose());
            disposeCallbacks.push(() => lowerBodyGeometry.dispose());
            disposeCallbacks.push(() => armGeometry.dispose());
            disposeCallbacks.push(() => handGeometry.dispose());
            disposeCallbacks.push(() => wingGeometry.dispose());
            disposeCallbacks.push(() => haloGeometry.dispose());
            disposeCallbacks.push(() => eyeGeometry.dispose());
            disposeCallbacks.push(() => sparkleGeometry.dispose());

            const auraNodes = createMagicActorAura(disposeCallbacks, {
                position: [0, 0.18 * scale, 0.02],
                lightColor: variantConfig.LIGHT_COLOR,
                lightIntensity: variantConfig.LIGHT_INTENSITY,
                lightDistance: variantConfig.LIGHT_DISTANCE,
                coreColor: variantConfig.AURA_CORE_COLOR,
                coreRadius: 0.12 * scale,
                coreOpacity: variantConfig.GLOW_CORE_OPACITY,
                auraColor: variantConfig.AURA_COLOR,
                auraRadius: 0.24 * scale,
                auraOpacity: variantConfig.GLOW_AURA_OPACITY,
                auraScale: [1.0, 1.18, 0.96],
                shellColor: variantConfig.AURA_SHELL_COLOR,
                shellRadius: 0.34 * scale,
                shellOpacity: variantConfig.GLOW_SHELL_OPACITY,
                shellScale: [1.0, 1.3, 0.92],
                haloColor: variantConfig.AURA_HALO_COLOR,
                haloInnerRadius: 0.12 * scale,
                haloOuterRadius: 0.22 * scale,
                haloOpacity: variantConfig.GLOW_HALO_OPACITY,
                haloPosition: [0, 0.04 * scale, -0.12 * scale],
                pulseSpeed: variantConfig.PULSE_SPEED,
                lightPulse: 0.28,
                corePulse: 0.08,
                auraPulse: 0.06,
                shellPulse: 0.05,
                haloPulse: 0.05,
                auraScalePulse: 0.1,
                shellScalePulse: 0.12,
            });
            bodyPivot.add(auraNodes.group);

            const torso = new THREE.Mesh(torsoGeometry, bodyMaterial);
            torso.position.set(0, 0.18 * scale, 0.02 * scale);
            torso.scale.set(0.88, 1.28, 0.72);
            bodyPivot.add(torso);

            const head = new THREE.Mesh(headGeometry, bodyMaterial);
            head.position.set(0, 0.42 * scale, 0.05 * scale);
            head.scale.set(0.95, 1.02, 0.92);
            bodyPivot.add(head);

            const lowerBody = new THREE.Mesh(lowerBodyGeometry, lowerBodyMaterial);
            lowerBody.position.set(0, -0.02 * scale, 0.01 * scale);
            bodyPivot.add(lowerBody);

            const leftEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
            leftEye.position.set(-0.045 * scale, 0.015 * scale, 0.1 * scale);
            head.add(leftEye);
            const rightEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
            rightEye.position.set(0.045 * scale, 0.015 * scale, 0.1 * scale);
            head.add(rightEye);

            const haloPivot = new THREE.Group();
            haloPivot.position.set(0, 0.64 * scale, 0);
            bodyPivot.add(haloPivot);
            const haloMesh = new THREE.Mesh(haloGeometry, haloMaterial);
            haloMesh.rotation.x = Math.PI * 0.25;
            haloPivot.add(haloMesh);

            const leftArmPivot = new THREE.Group();
            leftArmPivot.position.set(-0.11 * scale, 0.25 * scale, 0.03 * scale);
            bodyPivot.add(leftArmPivot);
            const leftArm = new THREE.Mesh(armGeometry, limbMaterial);
            leftArm.position.y = -0.5 * baseConfig.ARM_LENGTH * scale;
            leftArm.rotation.z = 0.16;
            leftArmPivot.add(leftArm);
            const leftHand = new THREE.Mesh(handGeometry, limbMaterial);
            leftHand.position.set(0.02 * scale, -baseConfig.ARM_LENGTH * scale, 0.02 * scale);
            leftArmPivot.add(leftHand);

            const rightArmPivot = new THREE.Group();
            rightArmPivot.position.set(0.11 * scale, 0.25 * scale, 0.03 * scale);
            bodyPivot.add(rightArmPivot);
            const rightArm = new THREE.Mesh(armGeometry, limbMaterial);
            rightArm.position.y = -0.5 * baseConfig.ARM_LENGTH * scale;
            rightArm.rotation.z = -0.16;
            rightArmPivot.add(rightArm);
            const rightHand = new THREE.Mesh(handGeometry, limbMaterial);
            rightHand.position.set(-0.02 * scale, -baseConfig.ARM_LENGTH * scale, 0.02 * scale);
            rightArmPivot.add(rightHand);

            const buildWingSet = (side, depthOffset, widthScale, heightScale, material, xOffset, yOffset) => {
                const pivot = new THREE.Group();
                pivot.position.set(side * xOffset * scale, yOffset * scale, depthOffset * scale);
                bodyPivot.add(pivot);

                const wing = new THREE.Mesh(wingGeometry, material);
                wing.scale.set(side * baseConfig.WING_WIDTH * scale * widthScale, baseConfig.WING_HEIGHT * scale * heightScale, 1);
                wing.position.set(side * 0.01 * scale, 0, 0);
                wing.rotation.z = side > 0 ? -0.18 : 0.18;
                pivot.add(wing);

                return pivot;
            };

            const leftFrontWing = buildWingSet(-1, -0.03, 1, 1, wingMaterial, baseConfig.FRONT_WING_OFFSET_X, baseConfig.FRONT_WING_OFFSET_Y);
            const rightFrontWing = buildWingSet(1, -0.03, 1, 1, wingMaterial, baseConfig.FRONT_WING_OFFSET_X, baseConfig.FRONT_WING_OFFSET_Y);
            const leftRearWing = buildWingSet(-1, -0.09, 0.82, 0.88, rearWingMaterial, baseConfig.REAR_WING_OFFSET_X, baseConfig.REAR_WING_OFFSET_Y);
            const rightRearWing = buildWingSet(1, -0.09, 0.82, 0.88, rearWingMaterial, baseConfig.REAR_WING_OFFSET_X, baseConfig.REAR_WING_OFFSET_Y);

            const sparkleGroup = new THREE.Group();
            bodyPivot.add(sparkleGroup);
            const sparkles = [];
            for (let index = 0; index < baseConfig.SPARKLE_COUNT; index += 1) {
                const sparkle = new THREE.Mesh(sparkleGeometry, sparkleMaterial);
                sparkleGroup.add(sparkle);
                sparkles.push({
                    mesh: sparkle,
                    phase: (index / Math.max(1, baseConfig.SPARKLE_COUNT)) * Math.PI * 2,
                });
            }
            ...
            if (variantConfig.WAND_ENABLED) {
                const wandMaterial = registerMaterial(new THREE.MeshPhysicalMaterial({
                    color: 0xffffff,
                    emissive: variantConfig.AURA_HALO_COLOR,
                    emissiveIntensity: 0.8,
                    roughness: 0.32,
                    metalness: 0.08,
                }));
                const wandTipMaterial = registerMaterial(new THREE.MeshBasicMaterial({
                    color: variantConfig.AURA_COLOR,
                    transparent: true,
                    opacity: 0.9,
                    blending: THREE.AdditiveBlending,
                    depthWrite: false,
                }));
                const wandGeometry = new THREE.CylinderGeometry(0.015 * scale, 0.02 * scale, 0.44 * scale, 8);
                const wandTipGeometry = new THREE.SphereGeometry(0.045 * scale, 10, 10);
                disposeCallbacks.push(() => wandGeometry.dispose());
                disposeCallbacks.push(() => wandTipGeometry.dispose());

                wandPivot = new THREE.Group();
                wandPivot.position.set(0.13 * scale, 0.1 * scale, 0.06 * scale);
                bodyPivot.add(wandPivot);

                const wand = new THREE.Mesh(wandGeometry, wandMaterial);
                wand.position.set(0, -0.12 * scale, 0);
                wand.rotation.z = -Math.PI / 7;
                wandPivot.add(wand);

                wandTip = new THREE.Mesh(wandTipGeometry, wandTipMaterial);
                wandTip.position.set(0.12 * scale, 0.09 * scale, 0.02 * scale);
                wandPivot.add(wandTip);

                accentLight = registerLight(new THREE.PointLight(
                    variantConfig.ACCENT_LIGHT_COLOR,
                    variantConfig.ACCENT_LIGHT_INTENSITY,
                    variantConfig.ACCENT_LIGHT_DISTANCE,
                    2
                ));
                accentLight.position.copy(wandTip.position);
                wandPivot.add(accentLight);
            } else {
                accentLight = registerLight(new THREE.PointLight(
                    variantConfig.ACCENT_LIGHT_COLOR,
                    variantConfig.ACCENT_LIGHT_INTENSITY,
                ...
                accentLight.position.set(0, 0.62 * scale, 0.02 * scale);
                bodyPivot.add(accentLight);
            }
            ...
            return {
                ...
                variantConfig,
                visibilityAlpha: 1,
                motionScratch: new THREE.Vector3(),
                lastRootPosition: new THREE.Vector3(),
                hasMotionSample: false,
                wingPhaseOffset: Math.random() * Math.PI * 2,
            };
        }

        function setMagicActorVisibilityAlpha(rig, alpha) {
            if (!rig) {
                return;
            }

            const visibilityAlpha = THREE.MathUtils.clamp(alpha, 0, 1);
            rig.visibilityAlpha = visibilityAlpha;
            if (rig.auraNodes) {
                rig.auraNodes.visibilityAlpha = visibilityAlpha;
            }

            rig.fadeMaterials?.forEach((material) => {
                const baseOpacity = material.userData.magicBaseOpacity ?? material.opacity ?? 1;
                if (material.transparent || baseOpacity < 1) {
                    material.transparent = true;
                    material.opacity = baseOpacity * visibilityAlpha;
                }
            });

            rig.fadeLights?.forEach((light) => {
                const baseIntensity = light.userData.magicBaseIntensity ?? light.intensity ?? 1;
                light.intensity = baseIntensity * visibilityAlpha;
            });
        }

        function updateMagicFairyRig(rig, phaseTime, motionState = {}) {
            if (!rig?.root || !rig.bodyPivot) {
                return;
            }

            const baseConfig = CONFIG.FAIRY_VISUALS.BASE;
            const variantConfig = rig.variantConfig;
            const visibilityAlpha = rig.visibilityAlpha ?? 1;
            const hoverStrength = motionState.hoverStrength ?? 1;
            const wingIntensity = motionState.wingIntensity ?? variantConfig.WING_INTENSITY ?? 1;
            const castAmount = motionState.castAmount ?? 0;
            const bank = motionState.bank ?? 0;
            const lean = motionState.lean ?? 0.06;
            const bodyPulse = Math.sin((phaseTime * 3.2) + rig.wingPhaseOffset) * baseConfig.BODY_PULSE;
            const bob = Math.sin((phaseTime * 2.5) + rig.wingPhaseOffset) * baseConfig.HOVER_BOB_HEIGHT * hoverStrength;
            const swayX = Math.sin((phaseTime * 1.8) + (rig.wingPhaseOffset * 0.7)) * baseConfig.HOVER_SWAY_X * hoverStrength;
            const swayZ = Math.cos((phaseTime * 2.1) + (rig.wingPhaseOffset * 0.6)) * baseConfig.HOVER_SWAY_Z * hoverStrength;
            const wingPulse = Math.sin((phaseTime * baseConfig.WING_FLAP_SPEED * wingIntensity) + rig.wingPhaseOffset) * 0.5 + 0.5;
            const wingSweep = THREE.MathUtils.lerp(baseConfig.WING_SWEEP_MIN, baseConfig.WING_SWEEP_MAX, wingPulse);
            const rearWingSweep = wingSweep * 0.84;
            const armPulse = Math.sin((phaseTime * 6.8) + rig.wingPhaseOffset) * 0.5 + 0.5;

            rig.bodyPivot.position.set(swayX, bob, swayZ);
            rig.bodyPivot.rotation.x = lean + (bodyPulse * 0.35) + (castAmount * 0.12);
            rig.bodyPivot.rotation.z = (bank * 0.3) + Math.sin((phaseTime * 1.5) + rig.wingPhaseOffset) * 0.03;

            rig.leftFrontWing.rotation.y = -wingSweep;
            rig.leftFrontWing.rotation.z = -0.2 + (wingPulse * 0.18);
            rig.rightFrontWing.rotation.y = wingSweep;
            rig.rightFrontWing.rotation.z = 0.2 - (wingPulse * 0.18);

            rig.leftRearWing.rotation.y = -rearWingSweep;
            rig.leftRearWing.rotation.z = -0.28 + (wingPulse * 0.14);
            rig.rightRearWing.rotation.y = rearWingSweep;
            rig.rightRearWing.rotation.z = 0.28 - (wingPulse * 0.14);

            rig.leftArmPivot.rotation.x = 0.28 + (castAmount * 0.22) + (armPulse * 0.08);
            rig.leftArmPivot.rotation.z = 0.2 + (armPulse * 0.12);
            rig.rightArmPivot.rotation.x = 0.28 + (castAmount * 0.22) + ((1 - armPulse) * 0.08);
            rig.rightArmPivot.rotation.z = -0.2 - ((1 - armPulse) * 0.12);

            rig.head.rotation.y = Math.sin((phaseTime * 1.4) + rig.wingPhaseOffset) * 0.08;
            rig.head.rotation.x = -0.04 + (castAmount * 0.1);

            rig.haloPivot.rotation.y = phaseTime * 0.8;
            rig.haloMesh.rotation.z = Math.sin((phaseTime * 0.9) + rig.wingPhaseOffset) * 0.08;

            if (rig.wandPivot) {
                rig.wandPivot.rotation.z = (-Math.PI / 9) - (castAmount * 0.58) + Math.sin((phaseTime * 2.2) + rig.wingPhaseOffset) * 0.08;
                rig.wandPivot.rotation.x = 0.12 + (castAmount * 0.18);
            }

            rig.sparkles.forEach((sparkle, index) => {
                const orbit = phaseTime * (0.9 + (index * 0.07)) + sparkle.phase;
                const radius = baseConfig.SPARKLE_RADIUS + Math.sin(phaseTime * 1.4 + sparkle.phase) * 0.03;
                sparkle.mesh.position.set(
                    Math.cos(orbit) * radius,
                    0.18 + Math.sin((orbit * 1.6) + rig.wingPhaseOffset) * baseConfig.SPARKLE_HEIGHT,
                    Math.sin(orbit) * (radius * 0.6)
                );
                const sparkleScale = 0.65 + ((Math.sin((phaseTime * 5.4) + sparkle.phase) * 0.5 + 0.5) * 0.6);
                sparkle.mesh.scale.setScalar(sparkleScale);
                sparkle.mesh.material.opacity = sparkle.mesh.material.userData.magicBaseOpacity * visibilityAlpha * (0.55 + (sparkleScale * 0.45));
            });

            if (!rig.hasMotionSample) {
                rig.lastRootPosition.copy(rig.root.position);
                rig.hasMotionSample = true;
            }

            rig.motionScratch.copy(rig.root.position).sub(rig.lastRootPosition);
            rig.lastRootPosition.copy(rig.root.position);

            if ((motionState.faceMotion ?? true) && rig.motionScratch.lengthSq() > 0.000001) {
                const targetYaw = Math.atan2(rig.motionScratch.x, rig.motionScratch.z);
                rig.root.rotation.y = blendAngle(rig.root.rotation.y, targetYaw, baseConfig.MOTION_TURN_BLEND);
            }

            const motionBank = THREE.MathUtils.clamp(rig.motionScratch.x * 1.4, -0.18, 0.18);
            rig.root.rotation.z = bank + motionBank;

            if (rig.accentLight) {
                const baseIntensity = rig.accentLight.userData.magicBaseIntensity ?? rig.accentLight.intensity;
                rig.accentLight.intensity = baseIntensity * (0.82 + (wingPulse * 0.34) + (castAmount * 0.28)) * visibilityAlpha;
            }

            updateMagicActorAura(rig.auraNodes, phaseTime, {
                intensityBoost: motionState.auraBoost ?? 0,
                shellPulse: motionState.shellPulse ?? (castAmount * 0.3),
                haloBoost: motionState.haloBoost ?? (castAmount * 0.05),
                coreBoost: castAmount * 0.03,
            });
        }

        function disposeMagicFairyRig(rig) {
            if (!rig) {
                return;
            }

            anime.remove(rig.root?.position);
            anime.remove(rig.root?.rotation);
            if (rig.root?.parent) {
                rig.root.parent.remove(rig.root);
            }

            rig.disposeCallbacks?.forEach((dispose) => {
                try {
                    dispose?.();
                } catch (error) {
                    console.warn('Failed to dispose fairy visual resource cleanly.', error);
                }
            });
        }

        function startMagicActorTicker(updateCallback) {
            let active = true;
            let frameId = 0;
        ...
        function getGoblinSpriteWorldDimensions(bundle = goblinSpriteBundle) {
            if (!bundle?.manifest?.frameSize || !bundle?.manifest?.anchorPx) {
                return null;
            }

            const frameWidth = bundle.manifest.frameSize.width;
            const frameHeight = bundle.manifest.frameSize.height;
            const pixelsToWorld = CONFIG.GOBLIN_INTRO.SPRITE_WORLD_HEIGHT / Math.max(1, frameHeight);
            return {
                frameWidth,
                frameHeight,
                pixelsToWorld,
                worldHeight: CONFIG.GOBLIN_INTRO.SPRITE_WORLD_HEIGHT,
                worldWidth: frameWidth * pixelsToWorld,
                anchorOffsetX: ((frameWidth * 0.5) - bundle.manifest.anchorPx.x) * pixelsToWorld,
                anchorOffsetY: (bundle.manifest.anchorPx.y - (frameHeight * 0.5)) * pixelsToWorld,
            };
        }

        function createGoblinEyeGlowNodes(disposeCallbacks, dimensions) {
            const eyeGroup = new THREE.Group();
            const eyeY = ((dimensions.frameHeight * 0.24) - goblinSpriteBundle.manifest.anchorPx.y) * -dimensions.pixelsToWorld;
            eyeGroup.position.set(0, eyeY, CONFIG.GOBLIN_INTRO.SPRITE_WORLD_DEPTH_OFFSET + 0.03);

            const eyeGeometry = new THREE.SphereGeometry(0.022, 10, 10);
            const leftEyeMaterial = new THREE.MeshBasicMaterial({
                color: CONFIG.GOBLIN_INTRO.EYE_GLOW_COLOR,
                transparent: true,
                opacity: 0.8,
                blending: THREE.AdditiveBlending,
                depthWrite: false,
            ...
            const eyeX = dimensions.worldWidth * 0.108;

            const leftEye = new THREE.Mesh(eyeGeometry, leftEyeMaterial);
            leftEye.position.set(-eyeX, 0.008, 0);
            const rightEye = new THREE.Mesh(eyeGeometry, rightEyeMaterial);
            rightEye.position.set(eyeX, 0.008, 0);
            eyeGroup.add(leftEye, rightEye);

            const eyeLight = new THREE.PointLight(CONFIG.GOBLIN_INTRO.EYE_GLOW_COLOR, CONFIG.GOBLIN_INTRO.EYE_GLOW_INTENSITY, 0.8, 2);
            eyeLight.position.set(0, 0, 0.06);
            eyeGroup.add(eyeLight);

            disposeCallbacks.push(() => eyeGeometry.dispose());
            disposeCallbacks.push(() => leftEyeMaterial.dispose());
            disposeCallbacks.push(() => rightEyeMaterial.dispose());
        ...
        function createGoblinGlowNodes(disposeCallbacks, dimensions) {
            return createMagicActorAura(disposeCallbacks, {
                position: [0, CONFIG.GOBLIN_INTRO.GLOW_AURA_POSITION_Y, CONFIG.GOBLIN_INTRO.GLOW_AURA_POSITION_Z],
                lightColor: CONFIG.GOBLIN_INTRO.GLOW_LIGHT_COLOR,
                lightIntensity: CONFIG.GOBLIN_INTRO.GLOW_LIGHT_INTENSITY,
                lightDistance: CONFIG.GOBLIN_INTRO.GLOW_LIGHT_DISTANCE,
                coreColor: CONFIG.GOBLIN_INTRO.GLOW_CORE_COLOR,
                coreRadius: dimensions.worldHeight * 0.085,
                coreOpacity: CONFIG.GOBLIN_INTRO.GLOW_CORE_OPACITY,
                auraColor: CONFIG.GOBLIN_INTRO.GLOW_AURA_COLOR,
                auraRadius: dimensions.worldHeight * 0.28,
                auraOpacity: CONFIG.GOBLIN_INTRO.GLOW_AURA_OPACITY,
                auraScale: [1.06, 1.24, 0.88],
                shellColor: CONFIG.GOBLIN_INTRO.GLOW_SHELL_COLOR,
                shellRadius: dimensions.worldHeight * 0.38,
                shellOpacity: CONFIG.GOBLIN_INTRO.GLOW_SHELL_OPACITY,
                shellScale: [1.14, 1.42, 0.76],
                haloColor: CONFIG.GOBLIN_INTRO.GLOW_HALO_COLOR,
                haloInnerRadius: CONFIG.GOBLIN_INTRO.GLOW_HALO_INNER_RADIUS,
                haloOuterRadius: CONFIG.GOBLIN_INTRO.GLOW_HALO_OUTER_RADIUS,
                haloOpacity: CONFIG.GOBLIN_INTRO.GLOW_HALO_OPACITY,
                haloPosition: [0, 0.03, CONFIG.GOBLIN_INTRO.GLOW_HALO_DEPTH],
                pulseSpeed: CONFIG.GOBLIN_INTRO.GLOW_PULSE_SPEED,
                lightPulse: 0.22,
                corePulse: 0.08,
                auraPulse: 0.05,
                shellPulse: 0.04,
        ...
        function setGoblinSpriteFrame(rig, animationKey, frameIndex) {
            const animation = goblinSpriteBundle?.animations?.[animationKey];
            const spritePlane = rig?.spritePlane;
            if (!animation || !spritePlane?.material) {
                return;
            }

            const clampedFrameIndex = animation.loop
                ? ((frameIndex % animation.frameCount) + animation.frameCount) % animation.frameCount
                : THREE.MathUtils.clamp(frameIndex, 0, animation.frameCount - 1);
            if (rig.currentAnimationKey === animationKey && rig.currentFrameIndex === clampedFrameIndex) {
                return;
            }

            const atlasTexture = animation.texture;
            const atlasWidth = atlasTexture.image?.width || (animation.columns * goblinSpriteBundle.manifest.frameSize.width);
            const atlasHeight = atlasTexture.image?.height || (animation.rows * goblinSpriteBundle.manifest.frameSize.height);
            const cellWidth = goblinSpriteBundle.manifest.frameSize.width / atlasWidth;
            const cellHeight = goblinSpriteBundle.manifest.frameSize.height / atlasHeight;
            const insetX = 0.5 / atlasWidth;
            const insetY = 0.5 / atlasHeight;
            const column = clampedFrameIndex % animation.columns;
            const row = Math.floor(clampedFrameIndex / animation.columns);

            if (spritePlane.material.map !== atlasTexture) {
                spritePlane.material.map = atlasTexture;
                spritePlane.material.needsUpdate = true;
            }

            atlasTexture.repeat.set(
                Math.max(0.0001, cellWidth - (insetX * 2)),
                Math.max(0.0001, cellHeight - (insetY * 2))
            );
            ...
            rig.currentAnimationKey = animationKey;
            rig.currentFrameIndex = clampedFrameIndex;
        }

        function applyGoblinLoopAnimation(rig, animationKey, animationTimeSeconds) {
            const animation = goblinSpriteBundle?.animations?.[animationKey];
            if (!animation) {
                return;
            }

            const frameIndex = Math.floor(Math.max(0, animationTimeSeconds) * animation.fps) % animation.frameCount;
            setGoblinSpriteFrame(rig, animationKey, frameIndex);
        }

        function applyGoblinAnimationProgress(rig, animationKey, progress) {
            const animation = goblinSpriteBundle?.animations?.[animationKey];
            if (!animation) {
                return;
            }
            ...
            const frameIndex = Math.round(clampedProgress * (animation.frameCount - 1));
            setGoblinSpriteFrame(rig, animationKey, frameIndex);
        }

        function applyGoblinBillboardState(rig, {
            roll = 0,
            scaleX = 1,
            scaleY = 1,
        } = {}) {
            if (!rig?.billboard || !camera) {
                return;
            }

            rig.visualRoll = roll;
            rig.visualScaleX = scaleX;
            rig.visualScaleY = scaleY;
            rig.billboard.quaternion.copy(camera.quaternion);
            if (roll) {
                rig.billboard.rotateZ(roll);
            }
            rig.billboard.scale.set(scaleX, scaleY, 1);
        }

        function createGoblinIntroRig() {
            if (!goblinSpriteBundle || !scene) {
                return null;
            }

            const dimensions = getGoblinSpriteWorldDimensions();
            if (!dimensions) {
                return null;
            }

            const root = new THREE.Group();
            root.name = GOBLIN_SCENE_ROOT_NAME;
            root.rotation.set(0, 0, 0);
            const disposeCallbacks = [];
            const billboard = new THREE.Group();
            billboard.name = 'GoblinFairyBillboard';
            root.add(billboard);

            const spriteGeometry = new THREE.PlaneGeometry(dimensions.worldWidth, dimensions.worldHeight);
            const spriteMaterial = new THREE.MeshBasicMaterial({
                color: 0xffffff,
                transparent: true,
                opacity: CONFIG.GOBLIN_INTRO.SPRITE_OPACITY,
                alphaTest: CONFIG.GOBLIN_INTRO.SPRITE_ALPHA_TEST,
                depthWrite: false,
                side: THREE.DoubleSide,
                toneMapped: false,
            });
            const spritePlane = new THREE.Mesh(spriteGeometry, spriteMaterial);
            spritePlane.name = 'GoblinFairySprite';
            spritePlane.position.set(
                dimensions.anchorOffsetX,
                dimensions.anchorOffsetY,
                CONFIG.GOBLIN_INTRO.SPRITE_WORLD_DEPTH_OFFSET
            );
            spritePlane.frustumCulled = false;
            spritePlane.renderOrder = 24;
            billboard.add(spritePlane);
            disposeCallbacks.push(() => spriteGeometry.dispose());
            disposeCallbacks.push(() => spriteMaterial.dispose());

            const glowNodes = createGoblinGlowNodes(disposeCallbacks, dimensions);
            billboard.add(glowNodes.group);

            const eyeNodes = createGoblinEyeGlowNodes(disposeCallbacks, dimensions);
            billboard.add(eyeNodes.eyeGroup);
            ...
            const rig = {
                ...
                billboard,
                spritePlane,
                pointLight: glowNodes.mainLight,
                glowNodes,
                eyeNodes,
                disposeCallbacks,
                dimensions,
                ...
                visualScaleX: 1,
                visualScaleY: 1,
                facingVector: new THREE.Vector3(0, 0, -1),
            };
            applyGoblinBillboardState(rig);
            setGoblinSpriteFrame(rig, 'hover', 0);

            return rig;
        }

        function resetGoblinPose(rig) {
            applyGoblinBillboardState(rig, {
                roll: 0,
                scaleX: 1,
                scaleY: 1,
        ...
        function setGoblinFacing(rig, direction) {
            if (!rig?.facingVector || !direction || direction.lengthSq() < 0.000001) {
                return;
            }

            rig.facingVector.copy(direction).normalize();
        }

        function applyGoblinFlightPose(rig, progress, flapTime, bankDirection = 1, aggression = 0) {
            resetGoblinPose(rig);
            applyGoblinLoopAnimation(rig, 'hover', flapTime * 0.08);

            const bank = THREE.MathUtils.clamp(bankDirection, -1, 1) * THREE.MathUtils.lerp(0.04, CONFIG.GOBLIN_INTRO.SCREEN_ROLL_LIMIT, 0.35 + (progress * 0.65));
            const stretch = CONFIG.GOBLIN_INTRO.FLIGHT_SCALE_BOOST * (0.55 + aggression);
            applyGoblinBillboardState(rig, {
                roll: bank,
                scaleX: 1 + (stretch * 0.4),
                scaleY: 1 - (stretch * 0.24),
            });
        }

        function applyGoblinHoverPose(rig, hoverTime, tension = 0.45) {
            resetGoblinPose(rig);
            applyGoblinLoopAnimation(rig, 'hover', hoverTime);

            const pulse = Math.sin(hoverTime * 2.6) * 0.5 + 0.5;
            const roll = Math.sin(hoverTime * 1.85) * (CONFIG.GOBLIN_INTRO.HOVER_ROLL * (0.26 + (tension * 0.12)));
            const scalePulse = pulse * CONFIG.GOBLIN_INTRO.HOVER_SCALE_PULSE;
            applyGoblinBillboardState(rig, {
                roll,
                scaleX: 1 + scalePulse,
                scaleY: 1 - (scalePulse * 0.5),
            });
        }

        function applyGoblinStrikePose(rig, strikeProgress, attackTime, impactStrength = 0) {
            resetGoblinPose(rig);
            applyGoblinAnimationProgress(rig, 'attack', strikeProgress);

            const clampedProgress = THREE.MathUtils.clamp(strikeProgress, 0, 1);
            const impactPulse = Math.sin(attackTime * 8.4) * 0.5 + 0.5;
            const roll = THREE.MathUtils.lerp(0.03, -CONFIG.GOBLIN_INTRO.SCREEN_ROLL_LIMIT * 0.45, clampedProgress) - (impactStrength * 0.03);
            const scaleX = 1 + ((CONFIG.GOBLIN_INTRO.STRIKE_SCALE_X - 1) * clampedProgress) + (impactStrength * 0.02);
            const scaleY = 1 - ((1 - CONFIG.GOBLIN_INTRO.STRIKE_SCALE_Y) * clampedProgress) - (impactPulse * 0.01) - (impactStrength * 0.015);
            applyGoblinBillboardState(rig, {
                roll,
                scaleX,
                scaleY,
        ...
        function updateGoblinGlow(rig, phaseTime, { intensityBoost = 0, shellPulse = 0, eyeBoost = 0 } = {}) {
            if (!rig) {
                return;
            }

            const introConfig = CONFIG.GOBLIN_INTRO;
            const glowPulse = Math.sin(phaseTime * 0.92) * 0.5 + 0.5;
            updateMagicActorAura(rig.glowNodes, phaseTime, {
                intensityBoost,
                shellPulse,
                haloBoost: 0.04 + (shellPulse * 0.05),
                coreBoost: intensityBoost * 0.04,
            });

            if (rig.eyeNodes?.leftEye?.material) {
                rig.eyeNodes.leftEye.material.opacity = 0.52 + (glowPulse * 0.18) + (eyeBoost * 0.12);
            }
            if (rig.eyeNodes?.rightEye?.material) {
                rig.eyeNodes.rightEye.material.opacity = 0.52 + (glowPulse * 0.18) + (eyeBoost * 0.12);
            }
            if (rig.eyeNodes?.eyeLight) {
                rig.eyeNodes.eyeLight.intensity = introConfig.EYE_GLOW_INTENSITY + (glowPulse * 0.12) + eyeBoost;
            }
        }

        function evaluateQuadraticBezier(start, control, end, t, target) {
            const inverseT = 1 - t;
            return target
                .copy(start)
                .multiplyScalar(inverseT * inverseT)
                .add(goblinBezierScratchB.copy(control).multiplyScalar(2 * inverseT * t))
                .add(goblinBezierScratchC.copy(end).multiplyScalar(t * t));
        }

        function getGoblinLaunchStartPosition() {
            const targetZ = camera ? camera.position.z - CONFIG.GOBLIN_INTRO.SPAWN_DEPTH_FROM_CAMERA : 0;
            const projectedPosition = getElementWorldTarget(goblinLaunchBadge, targetZ, gameContainer);
            if (projectedPosition) {
                return projectedPosition;
            }

            return new THREE.Vector3(
                camera ? camera.position.x - 1.8 : -1.8,
                camera ? camera.position.y + 0.9 : 2.8,
                targetZ
            );
        }

        function getGoblinEntryStartPosition(target = goblinHoverScratch) {
            target.copy(getGoblinLaunchStartPosition());
            target.y += 0.04;
            return target;
        }

        function getGoblinRarityScale() {
            const rarityData = CONFIG.PET_RARITIES[currentPetRarityKey] || CONFIG.PET_RARITIES.common;
            return rarityData.cageScale || 1;
        }

        function getGoblinChainTargetPosition(target = goblinTargetScratch) {
            const rarityData = CONFIG.PET_RARITIES[currentPetRarityKey] || CONFIG.PET_RARITIES.common;

            if (cage?.chainAttachPoint) {
                cage.chainAttachPoint.getWorldPosition(target);
            } else {
                target.set(
                    cage?.position.x || 0,
                    (cage?.position.y || CONFIG.INITIAL_CAGE_BASE_Y) + getCageHookOffsetForScale(rarityData.cageScale) + getMountedLinkCenterOffsetY(),
                    cage?.position.z || 0
                );
            }
        ...
        function getGoblinLookTargetPosition(target = goblinLookTargetScratch) {
            const scale = getGoblinRarityScale();
            getGoblinChainTargetPosition(target);
            target.y += CONFIG.GOBLIN_INTRO.LOOK_TARGET_VERTICAL_OFFSET * scale;
            target.z += CONFIG.GOBLIN_INTRO.LOOK_TARGET_DEPTH_OFFSET * scale;
            return target;
        }

        function getGoblinEntryOrbitCenter(target = goblinMidScratch) {
            const scale = getGoblinRarityScale();
            getGoblinChainTargetPosition(target);
            target.y += CONFIG.GOBLIN_INTRO.ORBIT_VERTICAL_OFFSET * scale;
            return target;
        }

        function getGoblinOrbitPosition(progress, target = goblinPerchScratch) {
            const introConfig = CONFIG.GOBLIN_INTRO;
            const scale = getGoblinRarityScale();
            getGoblinEntryOrbitCenter(target);
            const orbitCenterX = target.x;
            const orbitCenterY = target.y;
            const orbitCenterZ = target.z;
            const angle = THREE.MathUtils.lerp(Math.PI * 1.08, -0.22, progress);

            target.set(
                orbitCenterX + (Math.cos(angle) * introConfig.ORBIT_RADIUS_X * scale),
                orbitCenterY + (Math.sin(progress * Math.PI) * introConfig.ORBIT_RADIUS_Y * scale),
                orbitCenterZ + (Math.sin(angle) * introConfig.ORBIT_RADIUS_Z * scale)
            );
            return target;
        }

        function getGoblinRightHoverPosition(target = goblinHoverScratch) {
            const scale = getGoblinRarityScale();
            getGoblinChainTargetPosition(target);
            target.x += CONFIG.GOBLIN_INTRO.RIGHT_HOVER_LATERAL_OFFSET * scale;
            target.y += CONFIG.GOBLIN_INTRO.RIGHT_HOVER_VERTICAL_OFFSET * scale;
            target.z += CONFIG.GOBLIN_INTRO.RIGHT_HOVER_DEPTH_OFFSET * scale;
            return target;
        }

        function getGoblinStrikeTargetPosition(target = goblinTargetScratch) {
            const scale = getGoblinRarityScale();
            getGoblinChainTargetPosition(target);
            target.y += CONFIG.GOBLIN_INTRO.STRIKE_TARGET_VERTICAL_OFFSET * scale;
            target.z += CONFIG.GOBLIN_INTRO.STRIKE_TARGET_DEPTH_OFFSET * scale;
            return target;
        }

        function getGoblinWindupPosition(hoverTarget, strikeTarget, progress, target = goblinMidScratch) {
            target.copy(hoverTarget);
            goblinDirectionScratch.copy(strikeTarget).sub(hoverTarget);
            if (goblinDirectionScratch.lengthSq() > 0.000001) {
                goblinDirectionScratch.normalize();
                target.addScaledVector(goblinDirectionScratch, -0.18 * progress);
            }
            target.y += 0.08 * progress;
            target.z += 0.04 * progress;
            return target;
        }

        function removeOrphanedGoblinObjects() {
            if (!scene) {
                return;
            }

            scene.children
                .filter((child) => child?.name === GOBLIN_SCENE_ROOT_NAME || child?.name === GOBLIN_LEGACY_SCENE_ROOT_NAME)
                .forEach((child) => scene.remove(child));
        }

        function disposeGoblinRig(rig) {
            if (!rig) {
                return;
            }
            ...
            if (Array.isArray(rig.disposeCallbacks)) {
                rig.disposeCallbacks.forEach((dispose) => {
                    try {
                        dispose?.();
                    } catch (error) {
                        console.warn('Failed to dispose Cage Pet Rescue goblin resource.', error);
                    }
                });
            }
        }

        function cleanupGoblinIntro({ shiftGameplayTimer = false, resolveValue } = {}) {
            roundIntroSequence += 1;
            const intro = activeGoblinIntro;
            activeGoblinIntro = null;

            if (!intro) {
                removeOrphanedGoblinObjects();
                stopRoundIntroTimer({ shiftGameplayTimer });
                return;
            }

            if (intro.timeline) {
                intro.timeline.pause();
            }

            if (intro.rig) {
                disposeGoblinRig(intro.rig);
            } else if (intro.root || intro.disposeCallbacks) {
                disposeGoblinRig(intro);
            }

            removeOrphanedGoblinObjects();
            stopRoundIntroTimer({ shiftGameplayTimer });

            if (resolveValue !== undefined && typeof intro.settle === 'function') {
                const settle = intro.settle;
                intro.settle = null;
                settle(resolveValue);
            }
        }

        function setActiveGoblinPhase(intro, nextPhase) {
            if (!intro) {
                return;
            }

            intro.phase = nextPhase;
            intro.phaseElapsedMs = 0;
            intro.impactApplied = false;
            if (nextPhase === 'rightHover') {
                intro.strikeCooldownMs = CONFIG.GOBLIN_INTRO.STRIKE_INTERVAL_MS;
            }
        }

        function triggerGoblinStrikeVisualEffect() {
            if (!scene || !cage) {
                return;
            }

            const strikeFlash = new THREE.PointLight(0xb9d9ff, 2.8, 18, 2);
            strikeFlash.position.copy(getGoblinStrikeTargetPosition(goblinTargetScratch));
            strikeFlash.position.y += 0.08;
            strikeFlash.position.z += 0.12;
            ...
            anime({
                ...
                easing: 'easeOutQuad',
                complete: () => {
                    if (strikeFlash.parent) {
                        strikeFlash.parent.remove(strikeFlash);
                    }
                }
            });
        }

        function triggerGoblinStrikeImpact() {
            if (!cage || gameState !== 'PLAYING') {
                return;
            }
            ...
            triggerGoblinStrikeVisualEffect();
            playSound('cageHit');
            animateScreenShake(introConfig.STRIKE_SCREEN_SHAKE_INTENSITY);

            chainStress += introConfig.STRIKE_STRESS_INCREASE * rarityStressMultiplier;
            chainStress = Math.min(chainStress, CONFIG.MAX_STRESS);
            updateHUD();
            ...
            isJolting = true;
            animateCageShakeMotion({
                intensityMultiplier: introConfig.STRIKE_SHAKE_INTENSITY,
                duration: 260,
                petJumpHeight: 0.16,
                complete: () => {
                    isJolting = false;
        ...
        function updateActiveGoblinAttacker(time) {
            const intro = activeGoblinIntro;
            if (!intro?.rig || !intro.rig.root || !cage) {
                return;
            }

            if (gameState !== 'PLAYING' || isGamePaused) {
                intro.lastUpdateTime = time;
                return;
            }

            const introConfig = CONFIG.GOBLIN_INTRO;
            const deltaMs = Math.min(50, Math.max(0, time - (intro.lastUpdateTime || time)));
            intro.lastUpdateTime = time;
            intro.phaseElapsedMs += deltaMs;
            intro.totalElapsedMs += deltaMs;

            const rig = intro.rig;
            const phaseTime = intro.totalElapsedMs / 1000;

            getGoblinEntryOrbitCenter(rig.orbitCenter);
            getGoblinRightHoverPosition(rig.hoverTarget);
            getGoblinStrikeTargetPosition(rig.strikeTarget);
            getGoblinLookTargetPosition(rig.lookTarget);

            switch (intro.phase) {
                case 'entryOrbit': {
                    const totalEntryMs = introConfig.ENTRY_APPROACH_MS + introConfig.ORBIT_DURATION_MS;
                    if (intro.phaseElapsedMs < introConfig.ENTRY_APPROACH_MS) {
                        const progress = THREE.MathUtils.clamp(intro.phaseElapsedMs / introConfig.ENTRY_APPROACH_MS, 0, 1);
                        getGoblinOrbitPosition(0.1, rig.motionTarget);
                        goblinMidScratch.copy(rig.startPosition).lerp(rig.motionTarget, 0.5);
                        goblinMidScratch.y += introConfig.ENTRY_ARC_HEIGHT;

                        evaluateQuadraticBezier(rig.startPosition, goblinMidScratch, rig.motionTarget, progress, rig.root.position);
                        evaluateQuadraticBezier(
                            rig.startPosition,
                            goblinMidScratch,
                            rig.motionTarget,
                            Math.min(1, progress + 0.02),
                            goblinLookTargetScratch
                        );

                        goblinLookTargetScratch.sub(rig.root.position);
                        setGoblinFacing(rig, goblinLookTargetScratch);
                        applyGoblinFlightPose(rig, progress, phaseTime * 11, 1, 0.12);
                        updateGoblinGlow(rig, phaseTime * 1.3, {
                            intensityBoost: 0.08,
                            shellPulse: 0.1,
                            eyeBoost: 0.18,
                        });
                    ...
                    const orbitProgress = THREE.MathUtils.clamp(
                        (intro.phaseElapsedMs - introConfig.ENTRY_APPROACH_MS) / Math.max(1, introConfig.ORBIT_DURATION_MS),
                        0,
                        1
                    );
                    const blendToHover = THREE.MathUtils.clamp((orbitProgress - 0.72) / 0.28, 0, 1);

                    getGoblinOrbitPosition(orbitProgress, rig.motionTarget);
                    getGoblinOrbitPosition(Math.min(1, orbitProgress + 0.02), goblinLookTargetScratch);

                    if (blendToHover > 0) {
                        rig.motionTarget.lerp(rig.hoverTarget, blendToHover);
                        goblinLookTargetScratch.lerp(rig.hoverTarget, Math.min(1, blendToHover + 0.08));
                    }

                    rig.root.position.copy(rig.motionTarget);
                    goblinLookTargetScratch.sub(rig.root.position);
                    setGoblinFacing(rig, goblinLookTargetScratch);
                    applyGoblinFlightPose(
                        rig,
                        orbitProgress,
                        phaseTime * 12.5,
                        orbitProgress < 0.5 ? 1 : -0.55,
                        0.2
                    );
                    updateGoblinGlow(rig, phaseTime * 1.5, {
                        intensityBoost: 0.1,
                        shellPulse: 0.12,
                        eyeBoost: 0.24,
                    });

                    if (intro.phaseElapsedMs >= totalEntryMs) {
                        rig.root.position.copy(rig.hoverTarget);
                        setActiveGoblinPhase(intro, 'rightHover');
                    }
                    break;
                }

                case 'rightHover': {
                    intro.strikeCooldownMs -= deltaMs;

                    rig.root.position.copy(rig.hoverTarget);
                    rig.root.position.x += Math.sin(phaseTime * 1.65) * introConfig.HOVER_SWAY_X;
                    rig.root.position.y += Math.sin(phaseTime * 2.4) * introConfig.HOVER_BOB_HEIGHT;
                    rig.root.position.z += Math.cos(phaseTime * 1.9) * introConfig.HOVER_SWAY_Z;

                    goblinDirectionScratch.copy(rig.lookTarget).sub(rig.root.position);
                    setGoblinFacing(rig, goblinDirectionScratch);
                    applyGoblinHoverPose(rig, phaseTime, 0.5);
                    updateGoblinGlow(rig, phaseTime * 1.2, {
                        intensityBoost: 0.06,
                        shellPulse: 0.08,
                        eyeBoost: 0.14,
                    });

                    if (intro.strikeCooldownMs <= 0) {
                        setActiveGoblinPhase(intro, 'strikeWindup');
                    }
                    break;
                }

                case 'strikeWindup': {
                    const progress = THREE.MathUtils.clamp(intro.phaseElapsedMs / Math.max(1, introConfig.STRIKE_WINDUP_MS), 0, 1);
                    getGoblinWindupPosition(rig.hoverTarget, rig.strikeTarget, progress, rig.motionTarget);
                    rig.root.position.copy(rig.motionTarget);

                    goblinDirectionScratch.copy(rig.strikeTarget).sub(rig.root.position);
                    setGoblinFacing(rig, goblinDirectionScratch);
                    applyGoblinStrikePose(
                        rig,
                        THREE.MathUtils.lerp(introConfig.STRIKE_ANIMATION_WINDUP_START, introConfig.STRIKE_ANIMATION_WINDUP_END, progress),
                        phaseTime * 1.4,
                        0
                    );
                    updateGoblinGlow(rig, phaseTime * 1.4, {
                        intensityBoost: 0.12 + (progress * 0.06),
                        shellPulse: 0.1 + (progress * 0.08),
                        eyeBoost: 0.22 + (progress * 0.08),
                    });

                    if (progress >= 1) {
                        setActiveGoblinPhase(intro, 'strikeImpact');
                    }
                    break;
                }

                case 'strikeImpact': {
                    const totalImpactMs = introConfig.STRIKE_TRAVEL_MS + introConfig.STRIKE_IMPACT_MS;
                    if (intro.phaseElapsedMs < introConfig.STRIKE_TRAVEL_MS) {
                        const progress = THREE.MathUtils.clamp(intro.phaseElapsedMs / Math.max(1, introConfig.STRIKE_TRAVEL_MS), 0, 1);
                        getGoblinWindupPosition(rig.hoverTarget, rig.strikeTarget, 1, goblinMidScratch);
                        rig.root.position.lerpVectors(goblinMidScratch, rig.strikeTarget, progress);

                        goblinDirectionScratch.copy(rig.strikeTarget).sub(rig.root.position);
                        setGoblinFacing(rig, goblinDirectionScratch);
                        applyGoblinStrikePose(
                            rig,
                            THREE.MathUtils.lerp(introConfig.STRIKE_ANIMATION_IMPACT_START, introConfig.STRIKE_ANIMATION_IMPACT_END, progress),
                            phaseTime * 1.8,
                            progress * 0.25
                        );
                        updateGoblinGlow(rig, phaseTime * 1.6, {
                            intensityBoost: 0.18 + (progress * 0.12),
                            shellPulse: 0.12 + (progress * 0.16),
                            eyeBoost: 0.3 + (progress * 0.16),
                        });
                        break;
                    }

                    if (!intro.impactApplied) {
                        intro.impactApplied = true;
                        triggerGoblinStrikeImpact();
                        if (activeGoblinIntro !== intro) {
                            return;
                        }
                    }

                    const impactProgress = THREE.MathUtils.clamp(
                        (intro.phaseElapsedMs - introConfig.STRIKE_TRAVEL_MS) / Math.max(1, introConfig.STRIKE_IMPACT_MS),
                        0,
                        1
                    );
                    rig.root.position.copy(rig.strikeTarget);
                    rig.root.position.x += Math.sin(impactProgress * Math.PI * 4) * 0.03;
                    rig.root.position.y += Math.sin(impactProgress * Math.PI) * 0.02;

                    goblinDirectionScratch.copy(rig.lookTarget).sub(rig.root.position);
                    setGoblinFacing(rig, goblinDirectionScratch);
                    applyGoblinStrikePose(rig, introConfig.STRIKE_ANIMATION_RECOVER_START, phaseTime * 2.2, 0.8 - (impactProgress * 0.3));
                    updateGoblinGlow(rig, phaseTime * 1.8, {
                        intensityBoost: 0.24,
                        shellPulse: 0.22,
                        eyeBoost: 0.4,
                    });

                    if (intro.phaseElapsedMs >= totalImpactMs) {
                        setActiveGoblinPhase(intro, 'recover');
                    }
                    break;
                }

                case 'recover': {
                    const progress = THREE.MathUtils.clamp(intro.phaseElapsedMs / Math.max(1, introConfig.STRIKE_RECOVER_MS), 0, 1);
                    const easedProgress = 1 - Math.pow(1 - progress, 3);
                    rig.root.position.lerpVectors(rig.strikeTarget, rig.hoverTarget, easedProgress);
                    rig.root.position.y += Math.sin(progress * Math.PI) * 0.05;

                    goblinDirectionScratch.copy(rig.hoverTarget).sub(rig.root.position);
                    setGoblinFacing(rig, goblinDirectionScratch);
                    if (progress < 0.42) {
                        applyGoblinStrikePose(
                            rig,
                            THREE.MathUtils.lerp(introConfig.STRIKE_ANIMATION_RECOVER_START, introConfig.STRIKE_ANIMATION_RECOVER_END, progress / 0.42),
                            phaseTime * 2.05,
                            0.42 - (progress * 0.2)
                        );
                    } else {
                        applyGoblinFlightPose(rig, (progress - 0.42) / 0.58, phaseTime * 10.2, -1, 0.18);
                    }
                    updateGoblinGlow(rig, phaseTime * 1.35, {
                        intensityBoost: 0.12,
                        shellPulse: 0.12,
                        eyeBoost: 0.22,
                    });
        ...
        async function startGoblinRoundBehavior() {
            cleanupGoblinIntro({ resolveValue: false });

            if (!scene || !camera || !cage || gameState !== 'PLAYING') {
                return false;
            }

            const sequenceId = ++roundIntroSequence;
            const spriteBundle = await ensureGoblinSpriteAssetsLoaded();
            if (!spriteBundle || roundIntroSequence !== sequenceId || !scene || !camera || !cage || gameState !== 'PLAYING') {
                return false;
            }

            const rig = createGoblinIntroRig();
            if (!rig) {
                return false;
            }

            getGoblinEntryStartPosition(rig.startPosition);
            getGoblinRightHoverPosition(rig.hoverTarget);
            getGoblinStrikeTargetPosition(rig.strikeTarget);
            getGoblinLookTargetPosition(rig.lookTarget);

            rig.root.position.copy(rig.startPosition);
            goblinDirectionScratch.copy(rig.lookTarget).sub(rig.startPosition);
            setGoblinFacing(rig, goblinDirectionScratch);
            applyGoblinFlightPose(rig, 0, 0, 1, 0.1);
            updateGoblinGlow(rig, 0, {
                intensityBoost: 0.08,
                shellPulse: 0.08,
                eyeBoost: 0.18,
            });
            ...
            activeGoblinIntro = {
                ...
                strikeCooldownMs: CONFIG.GOBLIN_INTRO.STRIKE_INTERVAL_MS,
                impactApplied: false,
            };
        ...
        async function finalizeRoundActivation({
            startFreshTimer = false,
            initializeAudio = false,
            resetCloudSystem = false,
            scheduleLightning = false,
            applyRarityEffects = false,
            isLegendaryPet = false,
        } = {}) {
            setRoundControlsEnabled(false);

            if (startFreshTimer || realTimeTimerStart === 0) {
                resetGameplayTimerBaseline();
            } else {
                endGameplayTimerHold();
            }

            gameState = 'PLAYING';
            lastFrameTime = performance.now();
            setTimerDisplayFromGameTime();

            if (initializeAudio) {
                if (SOUNDS.ambient && SOUNDS.ambient.state() === 'loaded') {
                    if (!SOUNDS.ambient.playing()) SOUNDS.ambient.play();
                    SOUNDS.ambient.fade(SOUNDS.ambient.volume(), 0.3, 500);
                }
                if (SOUNDS.lava && SOUNDS.lava.state() === 'loaded') {
                    if (!SOUNDS.lava.playing()) SOUNDS.lava.play();
                    SOUNDS.lava.fade(SOUNDS.lava.volume(), 0.2, 500);
                }
                if (SOUNDS.chainSwing && SOUNDS.chainSwing.state() === 'loaded') {
                    if (!SOUNDS.chainSwing.playing()) SOUNDS.chainSwing.play();
                    SOUNDS.chainSwing.fade(SOUNDS.chainSwing.volume(), 0.1, 500);
                }
            }

            if (scheduleLightning) {
                scheduleNextLightningStrike({ initial: true });
            }
            ...
            if (applyRarityEffects && (isLegendaryPet || currentPetRarityKey === 'rare')) {
            ...
            void startGoblinRoundBehavior().catch((error) => {
                console.error('Failed to start Cage Pet Rescue goblin attack behavior.', error);
                cleanupGoblinIntro({ resolveValue: false });
            });
        }
        ...
        function animateCageShakeMotion({
            intensityMultiplier = 1,
            duration = 350,
            petJumpHeight = 0.3,
            complete = null,
        } = {}) {
            if (!cage || !pet || !pet.parent || pet.parent !== cage) {
            ...
            anime({
                ...
                x: [
                    { value: currentCagePos.x + (Math.random() - 0.5) * joltIntensityPos, duration: duration * 0.2 },
                    { value: currentCagePos.x - (Math.random() - 0.5) * joltIntensityPos, duration: duration * 0.5 },
                    { value: currentCagePos.x, duration: duration * 0.3 }
                ],
                y: [
                    { value: currentCagePos.y + (Math.random() - 0.5) * joltIntensityPos * 0.5, duration: duration * 0.2 },
                    { value: currentCagePos.y - (Math.random() - 0.5) * joltIntensityPos * 0.5, duration: duration * 0.5 },
                    { value: currentCagePos.y, duration: duration * 0.3 }
                ],
                z: [
                    { value: currentCagePos.z + (Math.random() - 0.5) * joltIntensityPos, duration: duration * 0.2 },
                    { value: currentCagePos.z - (Math.random() - 0.5) * joltIntensityPos, duration: duration * 0.5 },
                    { value: currentCagePos.z, duration: duration * 0.3 }
                ],
                easing: 'linear',
                duration
            });
            ...
            anime({
                ...
                z: [
                    { value: currentCageRot.z + (Math.random() - 0.5) * joltIntensityRot, duration: duration * 0.25 },
                    { value: currentCageRot.z - (Math.random() - 0.5) * joltIntensityRot, duration: duration * 0.5 },
                    { value: currentCageRot.z, duration: duration * 0.25 }
                ],
                easing: 'linear',
                duration,
                complete: () => {
            ...
            chainLinks.forEach((link, index) => {
                ...
                anime({
                    ...
                    x: [
                        { value: initialChainLinkTransforms[index].position.x + (Math.random() - 0.5) * joltIntensityPos, duration: duration * 0.2 },
                        { value: initialChainLinkTransforms[index].position.x - (Math.random() - 0.5) * joltIntensityPos, duration: duration * 0.5 },
                        { value: initialChainLinkTransforms[index].position.x, duration: duration * 0.3 }
                    ],
                    y: [
                        { value: initialChainLinkTransforms[index].position.y + (Math.random() - 0.5) * joltIntensityPos * 0.5, duration: duration * 0.2 },
                        { value: initialChainLinkTransforms[index].position.y - (Math.random() - 0.5) * joltIntensityPos * 0.5, duration: duration * 0.5 },
                        { value: initialChainLinkTransforms[index].position.y, duration: duration * 0.3 }
                    ],
                    z: [
                        { value: initialChainLinkTransforms[index].position.z + (Math.random() - 0.5) * joltIntensityPos * 0.2, duration: duration * 0.2 },
                        { value: initialChainLinkTransforms[index].position.z, duration: duration * 0.8 }
                    ],
                    easing: 'linear',
                    duration
                });
                ...
                anime({
                    ...
                    z: [
                        { value: initialChainLinkTransforms[index].rotation.z + (Math.random() - 0.5) * joltIntensityRot, duration: duration * 0.25 },
                        { value: initialChainLinkTransforms[index].rotation.z - (Math.random() - 0.5) * joltIntensityRot, duration: duration * 0.5 },
                        { value: initialChainLinkTransforms[index].rotation.z, duration: duration * 0.25 }
                    ],
                    easing: 'linear',
                    duration
                });
        ...
        function animateCageJolt(isLegendaryJolt = false) {
            ...
            if (isLegendaryJolt) {
                animateScreenShake(intensityMultiplier, true);
            }
            ...
            animateCageShakeMotion({
                intensityMultiplier,
                duration: isLegendaryJolt ? 450 : 350,
                petJumpHeight: 0.3 * intensityMultiplier,
                complete: () => {
                    isJolting = false;
                }
            });
        }
        function animateStressFairy() {
            if (!scene || !cage || stressFairyObject) {
                return;
            }

            actionButtons.stressFairy.disabled = true;

            const fairyRig = createProceduralFairyRig('STRESS');
            if (!fairyRig) {
                actionButtons.stressFairy.disabled = false;
                return;
            }

            stressFairyObject = fairyRig.root;
            stressFairyObject.position.set(-5, cage.position.y + 2, 0);
            scene.add(stressFairyObject);
            setMagicActorVisibilityAlpha(fairyRig, 1);
            ...
            const stopTicker = startMagicActorTicker((timestamp) => {
                updateMagicFairyRig(fairyRig, timestamp / 1000, motionState);
            });

            const fadeState = { alpha: 1 };
            const orbitState = { progress: 0 };
            const cageY = cage.position.y + 0.5;
            const radiusX = 2.5;
            const radiusZ = 1.5;
            ...
            anime.timeline({
                ...
                duration: 4000,
                complete: () => {
                    const reduction = CONFIG.MAX_STRESS * CONFIG.STRESS_REDUCTION_AMOUNT;
                    chainStress = Math.max(0, chainStress - reduction);
                    updateHUD();
                    stopTicker();
                    disposeMagicFairyRig(fairyRig);
                    stressFairyObject = null;
                    actionButtons.stressFairy.disabled = false;
                }
            })
            .add({
                targets: stressFairyObject.position,
                x: 0,
                y: cageY,
                z: radiusZ,
                duration: 1000,
                easing: 'easeOutQuad',
                update: () => {
                    motionState.bank = -0.04;
                    motionState.castAmount = 0.18;
                    motionState.auraBoost = 0.18;
                }
            })
            .add({
                targets: orbitState,
                ...
                easing: 'linear',
                update: () => {
                    const angle = orbitState.progress * Math.PI;
                    stressFairyObject.position.set(
                        Math.sin(angle) * radiusX,
                        cageY + 0.2 + Math.sin(angle * 2.1) * 0.14,
                        Math.cos(angle) * radiusZ
                    );
                    motionState.bank = Math.cos(angle) * 0.06;
                    motionState.castAmount = 0.26;
                    motionState.auraBoost = 0.22;
                    motionState.shellPulse = 0.16;
                }
            }, '-=500')
            .add({
                targets: orbitState,
                ...
                update: () => {
                    ...
                    stressFairyObject.position.set(
                        Math.sin(angle) * radiusX,
                        cageY - 0.1 + Math.sin(angle * 2.1) * 0.12,
                        Math.cos(angle) * radiusZ
                    );
                    motionState.bank = Math.cos(angle) * 0.06;
                    motionState.castAmount = 0.22;
                    motionState.auraBoost = 0.2;
                    motionState.shellPulse = 0.14;
                }
            }, '-=500')
            .add({
                targets: stressFairyObject.position,
                ...
                easing: 'easeInQuad',
                update: () => {
                    motionState.bank = 0.05;
                    motionState.castAmount = 0.14;
                    motionState.auraBoost = 0.1;
                    motionState.shellPulse = 0.08;
                }
            }, '-=500')
            .add({
                targets: fadeState,
                ...
                easing: 'linear',
                update: () => {
                    setMagicActorVisibilityAlpha(fairyRig, fadeState.alpha);
                }
            }, '-=700');
        }
        ...
        function handlePetLost(reason) {
            ...
            animateScreenShake(3.0, true); // Higher intensity for chain breaking
        ...
        function animateScreenShake(intensityMultiplier = 1.0, force = false) {
            if (!camera || (!force && gameState !== 'PLAYING')) return;
        ...
        function startGame() {
            ...
            anime({
                ...
                complete: async () => {
                    ...
                    // Keep the interval object for compatibility, but timer rendering stays on the animation loop.
                    timerInterval = setInterval(() => {}, 1000);

                    try {
                        await finalizeRoundActivation({
                            startFreshTimer: true,
                            initializeAudio: true,
                            resetCloudSystem: true,
                            scheduleLightning: true,
                            applyRarityEffects: currentPetRarityKey === 'rare' || currentPetRarityKey === 'legendary',
                            isLegendaryPet: currentPetRarityKey === 'legendary'
                        });
                        debugLog(`Game starting with timer: ${gameTime}s`);
                    } catch (error) {
                        console.error('Failed to finalize Cage Pet Rescue round start.', error);
                        cleanupGoblinIntro({ resolveValue: false });
                        stopRoundIntroTimer();
                        resetGameplayTimerBaseline();
                        gameState = 'PLAYING';
        ...
        function transitionToNextPet(newRarityKey) {
            ...
            const ambientWasPlaying = Boolean(SOUNDS.ambient?.playing());
            ...
            if (isLegendaryPet) {
                ...
                const legendaryRevealDelay = ambientWasPlaying ? 2000 : 250;
                setTimeout(() => {
                    createLegendaryLightningEffect(() => {
                        if (SOUNDS.legendaryPet) {
                            SOUNDS.legendaryPet.volume(0);
                            SOUNDS.legendaryPet.play();
                            SOUNDS.legendaryPet.fade(0, 0.7, 1500);
                        }
                    });
                }, legendaryRevealDelay);
            } else {
            ...
            Promise.all(fadeOutPromises).then(() => {
                ...
                anime({
                    ...
                    opacity: [0, 1],
                    duration: isLegendaryPet ? 1200 : 900,
                    delay: isLegendaryPet ? 4000 : 0,
                    easing: isLegendaryPet ? 'easeOutQuad' : 'linear',
                    complete: async () => {
                        ...
                        try {
                            await finalizeRoundActivation({
                                ...
                                resetCloudSystem: false,
                                scheduleLightning: true,
                                applyRarityEffects: true,
                                isLegendaryPet
                            });
                        } catch (error) {
                            console.error('Failed to finalize Cage Pet Rescue round transition.', error);
                            cleanupGoblinIntro({ resolveValue: false });
                            stopRoundIntroTimer();
                            endGameplayTimerHold();
                            gameState = 'PLAYING';
        ...
        function loseGame(reason) {
            ...
            mathProblemTimers.forEach((timerId, index) => { if (timerId) { clearTimeout(timerId); mathProblemTimers[index] = null; } });
            gameState = 'LOSE';
            clearInterval(timerInterval); if (lightningTimeoutId) clearTimeout(lightningTimeoutId); clearCloudSpawnTimer();
        ...
        function resetGameState() {
            ...
            if(timerInterval) clearInterval(timerInterval); if (lightningTimeoutId) clearTimeout(lightningTimeoutId); clearCloudSpawnTimer();
            isJolting = false;
        ...
        function setupEventListeners() {
            ...
            async function animateMathWizardFairy() {
                ...
                const fairyRig = createProceduralFairyRig('WIZARD');
                if (!fairyRig) {
                    return;
                }

                const cleanupWizardFairy = () => {
                ...
                const stopWizardTicker = startMagicActorTicker((timestamp) => {
                    updateMagicFairyRig(fairyRig, timestamp / 1000, wizardMotionState);
                });

                const visibilityState = { alpha: 1 };
                ...
                await new Promise(resolve => {
                    anime({
                        ...
                        update: anim => {
                            ...
                            wizardMotionState.bank = -0.06;
                            wizardMotionState.castAmount = 0.2;
                            wizardMotionState.auraBoost = 0.24;
                            wizardMotionState.shellPulse = 0.18;
                        },
                        complete: resolve
                    });
                });
                ...
                wizardMotionState.bank = 0;
                wizardMotionState.castAmount = 0.16;
                ...
                wizardMotionState.auraBoost = 0.28;
                wizardMotionState.shellPulse = 0.2;
                ...
                await new Promise(resolve => {
                    anime({
                        ...
                        update: () => {
                            wizardMotionState.auraBoost = 0.32;
                            wizardMotionState.shellPulse = 0.24;
                            wizardMotionState.castAmount = 0.72;
                        },
                        complete: resolve
                    });
                });
                ...
                wizardMotionState.castAmount = 1;
                wizardMotionState.auraBoost = 0.36;
                wizardMotionState.shellPulse = 0.28;
                ...
                for (let i = 0; i < totalOrbs; i++) {
                    ...
                    if (fairyRig.wandTip) {
                        fairyRig.wandTip.getWorldPosition(orb.position);
                    } else {
                ...
                await new Promise(resolve => {
                    anime({
                        ...
                        easing: 'easeInOutCubic',
                        update: () => {
                            wizardMotionState.castAmount = 0.32;
                            wizardMotionState.auraBoost = 0.2;
                            wizardMotionState.shellPulse = 0.14;
                        },
                        complete: resolve
                    });
                });
                ...
                await new Promise(resolve => {
                    ...
                    easing: 'easeInSine',
                    update: anim => {
                        wizardMotionState.castAmount = 0.1;
                        wizardMotionState.auraBoost = 0.08;
                        wizardMotionState.shellPulse = 0.06;
                        visibilityState.alpha = 1 - (anim.progress / 100);
                        setMagicActorVisibilityAlpha(fairyRig, visibilityState.alpha);
                    },
                    complete: () => {
                        stopWizardTicker();
                        cleanupWizardFairy();
        ...
        function animate(time) {
            ...
            updateActiveGoblinAttacker(time);
        ...
        function registerLocalDebugHooks() {
            ...
            window.__cagePetRescueDebug = {
                getGoblinState: () => ({
                    gameState,
                    introPhase: activeGoblinIntro?.phase || null,
                    goblinCount: activeGoblinIntro?.rig ? 1 : 0,
                    roundIntroActive: false,
                }),
                forceSuccessfulRescue: () => {
                    if (gameState !== 'PLAYING') {
                        return false;
                    }
                ...
                forcePetLost: (reason = 'Debug forced loss') => {
                    if (gameState !== 'PLAYING') {
                        return false;
                    }

                    handlePetLost(reason);
                    return true;
                },
            };
        }
